...
class SKILL_GALLOP_SLIDE_HYBRID_MotionGenerator(BaseMotionGenerator):
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Compression is achieved through X/Y positioning while maintaining ground contact (Z >= base).
        """
        ...
        if phase < 0.15:
            # Compression/gathering phase: rear legs pull back, front extend, all maintain ground contact
            progress = phase / 0.15
            smooth_progress = 0.5 * (1.0 - np.cos(np.pi * progress))
            ...
            if is_front:
                ...
                foot[2] = base_pos[2]  # Maintain ground contact
            else:
                # Rear legs compress rearward at ground level (compression visual from base descent)
                foot[0] = base_pos[0] - self.rear_compression_x * smooth_progress
                foot[2] = base_pos[2]  # Maintain ground contact, no downward offset
            ...
            # Regathering transition: legs pull inward, return to ground level
            progress = (phase - 0.65) / 0.15
            smooth_progress = 0.5 * (1.0 - np.cos(np.pi * progress))
            ...
            if is_front:
                ...
                # Smooth transition from glide lift to ground level
                foot[2] = base_pos[2] + self.glide_stance_lift * (1.0 - smooth_progress)
                
        else:
            # Return to compression: complete gathering, maintain ground contact
            progress = (phase - 0.80) / 0.20
            smooth_progress = 0.5 * (1.0 - np.cos(np.pi * progress))
            ...
            if is_front:
                ...
                # Rear legs remain in compressed rearward position at ground level
                foot[0] = base_pos[0] - self.rear_compression_x
                foot[2] = base_pos[2]  # Ground level, no downward offset
