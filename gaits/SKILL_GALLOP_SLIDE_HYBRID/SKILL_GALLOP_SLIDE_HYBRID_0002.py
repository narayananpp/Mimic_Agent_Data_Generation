...
class SKILL_GALLOP_SLIDE_HYBRID_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Motion parameters - scaled down for kinematic feasibility
        self.compression_depth = 0.05  # Reduced vertical compression distance
        self.front_extension_x = 0.10  # Reduced forward extension of front legs
        self.rear_compression_x = 0.05  # Reduced rearward pull of rear legs
        
        self.glide_stance_width_front = 0.11  # Reduced lateral extension during glide
        self.glide_stance_width_rear = 0.09   # Reduced lateral extension during glide
        self.glide_extension_x_front = 0.13   # Reduced forward extension during glide
        self.glide_extension_x_rear = 0.11    # Reduced rearward extension during glide
        self.glide_stance_lift = 0.005        # Minimal lift during glide to avoid penetration
        
        self.flight_height = 0.08  # Reduced peak height during ballistic phase
        
        # Velocity parameters - moderated for smoother motion
        ...
        # Pitch control parameters - reduced rates for smoother motion
        self.compression_pitch_rate = -0.5   # Nose-up during compression
        self.flight_pitch_rate = 0.8         # Nose-down during flight
        self.regather_pitch_rate = -0.4      # Nose-up during regathering
    ...
    def update_base_motion(self, phase, dt):
        ...
        if phase < 0.15:
            # Compression/gathering: decelerate, minimal descent, pitch nose-up
            progress = phase / 0.15
            smooth_progress = 0.5 * (1.0 - np.cos(np.pi * progress))
            vx = self.glide_final_vx * (1.0 - smooth_progress)
            vz = -0.15 * np.sin(np.pi * progress)  # Reduced descent rate
            ...
            if progress < 0.5:
                ...
                vz = -self.thrust_vz * 0.35 * descent_progress  # Reduced descent rate
            ...
            vz = -0.05 * np.sin(np.pi * progress)  # Minimal vertical oscillation
            pitch_rate = 0.0
            ...
            vz = -0.10 * smooth_progress  # Reduced descent
            pitch_rate = self.regather_pitch_rate * smooth_progress
            
        else:
            # Return to compression: final deceleration
            progress = (phase - 0.80) / 0.20
            smooth_progress = 0.5 * (1.0 - np.cos(np.pi * progress))
            vx = self.glide_final_vx * 0.8 * (1.0 - smooth_progress)
            vz = -0.08 * np.sin(np.pi * smooth_progress)  # Reduced descent
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        if phase < 0.15:
            ...
            if is_front:
                # Front legs extend forward slightly, minimal vertical change
                foot[0] = base_pos[0] + self.front_extension_x * smooth_progress
                foot[2] = base_pos[2]  # Keep at ground level
            else:
                # Rear legs compress inward and DOWN (toward body, not away)
                foot[0] = base_pos[0] - self.rear_compression_x * smooth_progress
                foot[2] = base_pos[2] - self.compression_depth * smooth_progress  # Fixed sign
            ...
            if is_front:
                ...
                foot[2] = base_pos[2] - self.compression_depth * (1.0 - flight_progress) + self.flight_height * smooth_flight
                
        elif phase < 0.35:
            # Extended landing transition (0.25-0.35) - increased duration
            ...
            if is_front:
                ...
                foot[2] = base_pos[2] + self.glide_stance_lift  # Minimal lift, not drop
            else:
                foot[0] = base_pos[0] + self.glide_extension_x_rear
            ...
            if is_front:
                ...
                foot[2] = base_pos[2] + self.glide_stance_lift * (1.0 - smooth_progress)
            else:
                ...
                # Transition from glide to compressed position
                foot[2] = base_pos[2] + self.glide_stance_lift * (1.0 - smooth_progress) - self.compression_depth * smooth_progress * 0.4
            ...
            if is_front:
                ...
                # Smoothly increase compression depth
                foot[2] = base_pos[2] - self.compression_depth * (0.4 + 0.6 * smooth_progress)
