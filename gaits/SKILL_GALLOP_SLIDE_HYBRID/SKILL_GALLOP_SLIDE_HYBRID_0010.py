...
class SKILL_GALLOP_SLIDE_HYBRID_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.flight_clearance_front = 0.035
        self.flight_clearance_rear = 0.030
        
        # Base state - initialize at nominal height
        self.t = 0.0
        self.root_pos = np.array([0.0, 0.0, 0.35])
    ...
    def update_base_motion(self, phase, dt):
        ...
        NO downward vz - height control via foot placement only.
        """
        ...
        # Phase 0.15-0.3: Thrust/launch (pitch timing shifted earlier)
        elif phase < 0.3:
            ...
            if local_phase < 0.5:
                ...
                vz = 0.0
            # Shift pitch earlier to reduce overlap with rear flight
            pitch_rate = 0.8 * np.sin(np.pi * (local_phase + 0.1))
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        """
        Compute foot position in BODY frame with refined Z offsets.
        """
        ...
        if phase < 0.15:
            ...
            if is_front:
                ...
                # Reduce Z offset and add slight inward position for knee comfort
                z_offset = 0.03
                if smooth > 0.7:
                    # Progressive reduction at max extension
                    z_offset = 0.03 * 0.7 + 0.02 * (smooth - 0.7) / 0.3
                foot[2] -= z_offset * smooth
                foot[1] += lateral_sign * (-0.02 * smooth)
            else:
        ...
        # Phase 0.15-0.3: Thrust/launch
            ...
            if is_front:
                ...
                # Reduced early thrust extension
                extension = 1.0 + 0.15 * smooth_profile
                foot[0] += self.front_extension_x * extension
                if local_phase > 0.2:
                    flight_local = (local_phase - 0.2) / 0.8
                    foot[2] += self.flight_clearance_front * np.sin(np.pi * flight_local) - 0.03 * (1.0 - flight_local)
                else:
                    foot[2] -= 0.03
            else:
                if local_phase < 0.55:
                    # Extension phase
                    extension_progress = local_phase / 0.55
                    ...
                    # Extended smooth transition zone with quadratic easing
                    blend_progress = (local_phase - 0.55) / 0.10
                    if blend_progress < 0.5:
                        blend = 2.0 * (blend_progress ** 2)
                    else:
                        blend = 1.0 - 2.0 * ((1.0 - blend_progress) ** 2)
                    extension_z = -0.04
                    flight_z_start = -0.02
                    foot[0] -= 0.08
                    foot[2] += extension_z * (1.0 - blend) + flight_z_start * blend
                else:
                    # Flight phase with reduced clearance for rear legs
                    flight_local = (local_phase - 0.65) / 0.35
                    foot[0] -= 0.08
                    foot[2] += self.flight_clearance_rear * np.sin(np.pi * flight_local) - 0.02
        ...
        # Phase 0.3-0.6: Slide/glide
            ...
            if local_phase < 0.40:
                ...
                if is_front:
                    ...
                    flight_z = self.flight_clearance_front * (1.0 - landing)
                    ...
                    flight_z = self.flight_clearance_rear * (1.0 - landing)
