...
class SKILL_GALLOP_SLIDE_HYBRID_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Base foot positions (BODY frame, neutral stance) - use original positions
        ...
        # Leg motion parameters - balanced for joint limits
    ...
    def update_base_motion(self, phase, dt):
        ...
        # Phase 0.0-0.15: Compression/gather - NO downward velocity
        if phase < 0.15:
            local_phase = phase / 0.15
            vx = 0.12 * local_phase
            vz = 0.0  # Compression via foot placement, not body descent
            pitch_rate = -0.15 * np.sin(np.pi * local_phase)
            
        # Phase 0.15-0.3: Thrust/launch - upward impulse only
        elif phase < 0.3:
        ...
        # Phase 0.75-1.0: Return to compression - NO downward velocity
        else:
            local_phase = (phase - 0.75) / 0.25
            vx = 0.08 * (1.0 - local_phase)
            vz = 0.0  # Compression via foot placement
        ...
        # Safety floor to prevent body sinking
        if self.root_pos[2] < self.nominal_height:
            self.root_pos[2] = self.nominal_height

    def compute_foot_position_body_frame(self, leg_name, phase):
        """
        Compute foot position in BODY frame with deeper Z offsets during stance.
        Compression achieved via foot geometry, not body descent.
        """
        ...
        if phase < 0.15:
            ...
            if is_front:
                ...
                foot[2] -= 0.05 * smooth  # Deeper stance for ground clearance
            else:
                # Rear compression: move forward AND up (shorter leg)
        ...
        # Phase 0.15-0.3: Thrust/launch
            ...
            local_phase = (phase - 0.15) / 0.15
            if is_front:
                extension = 1.0 + 0.2 * local_phase
                foot[0] += self.front_extension_x * extension
                if local_phase > 0.4:
                    flight_local = (local_phase - 0.4) / 0.6
                    foot[2] += self.flight_clearance * np.sin(np.pi * flight_local) - 0.05 * (1.0 - flight_local)
                else:
                    foot[2] -= 0.05
            else:
                if local_phase < 0.7:
                    ...
                    foot[0] -= 0.08
                    foot[2] += self.flight_clearance * np.sin(np.pi * flight_local) - 0.04 * (1.0 - np.sin(np.pi * flight_local))
                    
        # Phase 0.3-0.6: Slide/glide - extended landing transition
        elif phase < 0.6:
            local_phase = (phase - 0.3) / 0.3
            if local_phase < 0.35:
                # Extended landing transition with gentler descent
                landing = local_phase / 0.35
                ...
                if is_front:
                    ...
                    foot[2] += flight_z - 0.06 * smooth_land  # Deeper stance
                else:
                ...
                # Stable slide stance
                if is_front:
                    foot[0] += self.front_extension_x * 1.35
        ...
        # Phase 0.6-0.75: Gather inward
            ...
            smooth = (1.0 + np.cos(np.pi * local_phase)) / 2.0
            if is_front:
                foot[0] += self.front_extension_x * 1.35 * smooth
