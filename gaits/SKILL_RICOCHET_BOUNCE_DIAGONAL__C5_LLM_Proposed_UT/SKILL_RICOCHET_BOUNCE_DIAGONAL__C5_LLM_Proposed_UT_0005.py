...
class SKILL_RICOCHET_BOUNCE_DIAGONAL_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Use 0.12m margin for ground clearance
        max_foot_depth = max([-pos[2] for pos in initial_foot_positions_body.values()])
        self.nominal_base_height = max(0.35, max_foot_depth + 0.12)
        
        # Motion parameters - balanced between iteration 3 (best reward) and iteration 4 (zigzag success)
        self.compression_depth = 0.05  # Restored from iteration 3 for joint safety
        self.aerial_height = 0.08
        self.step_length_forward = 0.18
        self.step_length_lateral = 0.11  # Intermediate: between iter3 (0.10) and iter4 (0.12)
        self.leg_lift_height = 0.06
        
        # Velocity parameters - intermediate values for balance
        self.vx_cruise = 0.65
        self.vx_compression = 0.40
        self.vy_lateral = 0.48  # Intermediate: between iter3 (0.45) and iter4 (0.52)
        self.vz_launch = 0.65
        self.vz_compression = -0.30
        
        # Angular velocity parameters
        self.yaw_rate_compression = 2.2
        self.max_yaw_angle = np.deg2rad(30)
    ...
    def update_base_motion(self, phase, dt):
        ...
        Uses iteration 4's lateral velocity scaling (0.18) with iteration 3's safer magnitudes.
        """
        vx = 0.0
        vy = 0.0
        vz = 0.0
        ...
        height_correction = -0.5 * height_error
        ...
        if phase < 0.2:
            ...
            # Iteration 4's key insight: 0.18 scaling for meaningful lateral drift
            vy = -0.18 * self.vy_lateral  # -0.086 m/s leftward
        ...
        # Phase 0.2-0.4: Right-diagonal launch (aerial)
            ...
            vx = self.vx_cruise
            vy = self.vy_lateral  # +0.48 m/s rightward
        ...
        # Phase 0.4-0.6: Right compression
            ...
            compression_profile = np.sin(0.5 * np.pi * local_phase)
            vx = self.vx_compression
            # Sustained rightward drift with 0.18 scaling
            vy = 0.18 * self.vy_lateral * (1.0 - 0.5 * local_phase)  # +0.086 m/s rightward
            vz = self.vz_compression * compression_profile + height_correction
            yaw_rate = -self.yaw_rate_compression * np.sin(np.pi * local_phase)
        ...
        # Phase 0.6-0.8: Left-diagonal launch (aerial)
            ...
            vx = self.vx_cruise
            vy = -self.vy_lateral  # -0.48 m/s leftward
            launch_profile = 1.0 - 1.2 * local_phase
            vz = self.vz_launch * launch_profile + height_correction
            yaw_rate = -self.yaw_rate_compression * (1.0 - local_phase) * (1.0 - local_phase)
        ...
        # Phase 0.8-1.0: Landing preparation
            ...
            vx = self.vx_compression
            # Sustained leftward drift with 0.18 scaling
            vy = -0.18 * self.vy_lateral * (1.0 - 0.5 * local_phase)  # -0.086 m/s leftward
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Uses iteration 3's compression_depth (0.05m) and conservative repositioning
        with iteration 4's lateral motion structure. NO universal offset applied.
        ...
        # Phase 0.2-0.4: Right-diagonal launch (aerial)
            ...
            # Conservative repositioning: reduced from 0.35 to 0.30 for primary diagonal
            ...
            if is_left:
                ...
                # Reduced from 0.35 to 0.30 for workspace safety
                lateral_shift = self.step_length_lateral * 0.30 * (local_phase * local_phase)
                foot[1] += lateral_shift
        ...
        # NO universal offset - removed from iteration 4
