...
class SKILL_RICOCHET_BOUNCE_DIAGONAL_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.vy_lateral = 0.45  # Lateral velocity magnitude
        self.vz_launch = 0.65  # Upward launch velocity
        self.vz_compression = -0.30  # Downward compression velocity (moderate to prevent excessive descent)
        
        # Angular velocity parameters
        self.yaw_rate_compression = 2.0  # Yaw rate during compression (rad/s)
    ...
    def update_base_motion(self, phase, dt):
        ...
        if phase < 0.2:
            ...
            # Smooth compression using sine profile for gradual acceleration
            compression_profile = np.sin(0.5 * np.pi * local_phase)
            vx = self.vx_compression
            vy = -0.06 * self.vy_lateral  # Slight leftward drift
        ...
        # Phase 0.4-0.6: Right compression
            ...
            compression_profile = np.sin(0.5 * np.pi * local_phase)
            vx = self.vx_compression
            vy = 0.06 * self.vy_lateral * (1.0 - local_phase)  # Slight rightward then zero
            vz = self.vz_compression * compression_profile + height_correction
            # Smooth negative yaw
            yaw_rate = -self.yaw_rate_compression * np.sin(np.pi * local_phase)
        ...
        # Phase 0.8-1.0: Landing preparation
            ...
            vx = self.vx_compression
            vy = -0.06 * self.vy_lateral * (1.0 - local_phase)  # Leftward then zero
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        to represent leg retraction. This prevents ground penetration and keeps
        knees within valid flexion range.
        ...
        if phase < 0.2:
            local_phase = phase / 0.2
            # Smooth compression profile using sine for gradual motion
            ...
            if is_left:
                ...
                foot[0] -= 0.025 * compression_factor  # Slight rearward shift
            else:
                # Secondary support: lighter compression
                foot[2] -= 0.35 * self.compression_depth * compression_factor
        ...
        # Phase 0.2-0.4: Right-diagonal launch (aerial)
            ...
            # Reposition forward and rightward for landing - smooth transitions
            forward_shift = self.step_length_forward * 0.22 * (local_phase * local_phase)
            foot[0] += forward_shift
            
            if is_left:
                lateral_shift = self.step_length_lateral * 0.12 * (local_phase * local_phase)
        ...
        # Phase 0.4-0.6: Right compression
            ...
            if not is_left:
                ...
                foot[2] -= self.compression_depth * compression_factor
                foot[0] -= 0.025 * compression_factor
            else:
                # Secondary support: lighter compression
                foot[2] -= 0.35 * self.compression_depth * compression_factor
        ...
        # Phase 0.6-0.8: Left-diagonal launch (aerial)
            ...
            # Reposition forward and leftward for landing - smooth transitions
            forward_shift = self.step_length_forward * 0.22 * (local_phase * local_phase)
            foot[0] += forward_shift
        ...
        # Phase 0.8-1.0: Landing preparation
            ...
            local_phase = (phase - 0.8) / 0.2
            # Smooth transition to neutral stance using cosine for deceleration
            transition_factor = 0.5 * (1.0 - np.cos(np.pi * local_phase))
            
            if is_left:
                # Preparing for primary loading: slight pre-compression
                foot[2] -= 0.25 * self.compression_depth * transition_factor
                foot[0] += 0.015 * (1.0 - local_phase)
            else:
                # Secondary support positioning
                foot[2] -= 0.15 * self.compression_depth * transition_factor
