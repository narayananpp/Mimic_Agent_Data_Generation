...
class SKILL_RICOCHET_BOUNCE_DIAGONAL_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Calculate safe nominal height with increased margin to prevent ground penetration
        # Increased from 0.12m to 0.18m margin and minimum from 0.35m to 0.40m
        max_foot_depth = max([-pos[2] for pos in initial_foot_positions_body.values()])
        self.nominal_base_height = max(0.40, max_foot_depth + 0.18)
        
        # Motion parameters - optimized from iteration 5
        self.compression_depth = 0.05  # Maintains joint safety
        self.aerial_height = 0.08
        self.step_length_forward = 0.18
        self.step_length_lateral = 0.11  # Balanced for zigzag without joint violations
        self.leg_lift_height = 0.06
        
        # Velocity parameters - balanced for zigzag pattern and workspace safety
        self.vx_cruise = 0.65
        self.vx_compression = 0.40
        self.vy_lateral = 0.48  # Achieves bidirectional motion without excessive workspace reach
        self.vz_launch = 0.65
    ...
    def update_base_motion(self, phase, dt):
        ...
        Uses 0.18 lateral velocity scaling during compression for bidirectional zigzag.
        """
        vx = 0.0
        vy = 0.0
        vz = 0.0
        ...
        if phase < 0.2:
            ...
            # 0.18 scaling creates meaningful leftward drift
            vy = -0.18 * self.vy_lateral
        ...
        # Phase 0.4-0.6: Right compression
            ...
            vy = 0.18 * self.vy_lateral * (1.0 - 0.5 * local_phase)
            vz = self.vz_compression * compression_profile + height_correction
            yaw_rate = -self.yaw_rate_compression * np.sin(np.pi * local_phase)
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Compression: feet move UPWARD (negative z) to retract legs.
        Conservative repositioning (0.30 scaling) prevents joint limit violations.
        ...
        # Phase 0.2-0.4: Right-diagonal launch (aerial)
            ...
            # Conservative repositioning to prevent joint limits
