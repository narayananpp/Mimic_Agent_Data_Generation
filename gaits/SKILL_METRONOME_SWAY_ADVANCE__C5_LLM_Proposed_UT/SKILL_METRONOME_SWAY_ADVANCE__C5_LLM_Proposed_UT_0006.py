...
class SKILL_METRONOME_SWAY_ADVANCE_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.ground_clearance_margin = 0.035  # Optimized margin value
        
        # Base state - optimized height for startup transient elimination
        self.t = 0.0
        self.root_pos = np.array([0.0, 0.0, 0.39])  # Final optimized height
    ...
    def update_base_motion(self, phase, dt):
        ...
        # Phase-advanced profile with enhanced initial boost for startup transient
        if 0.0 <= phase < 0.25:
            # Right sway - right legs compressing, base should rise
            sway_progress = phase / 0.25
            # Phase-advanced sine with enhanced startup boost
            vz_base = 0.12 * np.sin(np.pi * min(sway_progress + 0.15, 1.0))
            # Enhanced initial boost for first 5% of phase to eliminate startup transient
            if phase < 0.05:
                vz = vz_base + 0.05  # Increased from 0.03 to 0.05
            else:
                vz = vz_base
        elif 0.25 <= phase < 0.5:
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        if is_compressed:
            # Enhanced margin scaling for startup phase (first 10% of cycle)
            if phase < 0.1:
                # Higher minimum scale during startup for right legs
                margin_scale = max(smooth_factor, 0.8)
            else:
                # Standard scaling after startup
