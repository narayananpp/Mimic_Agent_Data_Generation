...
class SKILL_METRONOME_SWAY_ADVANCE_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Ground clearance safety margin - increased substantially
        self.ground_clearance_margin = 0.055  # Increased from 0.03m to 0.055m
        
        # Base state - initialize with higher base height for ground clearance
        self.t = 0.0
        self.root_pos = np.array([0.0, 0.0, 0.42])  # Increased from 0.30m to 0.42m
    ...
    def update_base_motion(self, phase, dt):
        ...
        # Increased magnitude and added pre-emptive rise at phase boundaries
        if 0.0 <= phase < 0.25:
            # Right sway - right legs compressing, base should rise
            sway_progress = phase / 0.25
            # Phase-advanced sine to start rising earlier
            vz = 0.10 * np.sin(np.pi * min(sway_progress + 0.15, 1.0))
        elif 0.25 <= phase < 0.5:
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        # Increased margin for better ground clearance
