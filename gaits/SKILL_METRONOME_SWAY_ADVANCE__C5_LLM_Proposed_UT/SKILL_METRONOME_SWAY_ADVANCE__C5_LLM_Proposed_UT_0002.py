...
class SKILL_METRONOME_SWAY_ADVANCE_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Leg compression parameters - reduced to prevent ground penetration
        self.compression_amount = 0.035  # Vertical compression for loaded legs (reduced from 0.06)
        self.extension_amount = 0.025  # Vertical extension for unloaded legs (reduced from 0.04)
        self.lateral_shift_amount = 0.02  # Lateral shift in body frame
        
        # Ground clearance safety margin - lift feet higher in body frame
        self.ground_clearance_margin = 0.03  # Additional negative z offset for safety
        
        # Base state - initialize with proper height for ground clearance
        self.t = 0.0
        self.root_pos = np.array([0.0, 0.0, 0.30])  # Start at 0.30m height
    ...
    def update_base_motion(self, phase, dt):
        ...
        # Vertical velocity: CRITICAL FIX - rise during sway to compensate for leg compression
        # When legs compress, base must rise to maintain ground clearance
        if 0.0 <= phase < 0.25:
            # Right sway - right legs compressing, base should rise
            sway_progress = phase / 0.25
            vz = 0.08 * np.sin(np.pi * sway_progress)  # Rise during compression
        elif 0.25 <= phase < 0.5:
            # Neutral surge 1 - settling back down
            surge_progress = (phase - 0.25) / 0.25
            vz = -0.04 * np.sin(np.pi * surge_progress)  # Gentle descent
        elif 0.5 <= phase < 0.75:
            # Left sway - left legs compressing, base should rise
            sway_progress = (phase - 0.5) / 0.25
            vz = 0.08 * np.sin(np.pi * sway_progress)  # Rise during compression
        else:
            # Neutral surge 2 - settling back down
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        # Apply ground clearance safety margin - make z more negative (extend legs more)
        foot[2] -= self.ground_clearance_margin
