...
class SKILL_INVERTED_WAVE_HANDSTAND_MotionGenerator(BaseMotionGenerator):
    ...
    - Vertical compensation applied to prevent ground penetration during carving
    """
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Base foot positions in body frame with vertical clearance adjustment
        self.base_feet_pos_body = {}
        for k, v in initial_foot_positions_body.items():
            foot_pos = v.copy()
            # Lift front legs slightly to provide clearance during roll oscillations
            if k.startswith('F'):
                foot_pos[2] += 0.025  # 2.5 cm additional clearance for front legs
        ...
        # Rear leg lateral wave parameters (reduced amplitude)
        self.rear_lateral_amplitude = 0.08  # Reduced from 0.12 to reduce penetration risk
        
        # Vertical compensation factor for lateral sweeps
        self.lateral_z_compensation = 0.08  # Lift foot slightly during lateral sweep
        
        # Base angular velocity amplitudes (reduced roll rate)
        self.roll_rate_amp = 0.25  # Reduced from 0.4 to limit roll tilt
        self.yaw_rate_amp = 0.5   # Maintained for carving effect
        ...
        # Base vertical velocity amplitude for height modulation
        self.vz_amp = 0.08  # m/s subtle lift during maximum tilt phases
    ...
    def update_base_motion(self, phase, dt):
        ...
        Added vertical velocity modulation to counteract roll-induced sinking.
        
        Phase cycle:
        - [0.0, 0.25]: Left carve initiation (negative roll rate, positive yaw rate, upward vz)
        - [0.25, 0.5]: Left carve peak then transition (roll/yaw rates reduce, vz reduces)
        - [0.5, 0.75]: Right carve initiation (positive roll rate, negative yaw rate, upward vz)
        - [0.75, 1.0]: Right carve peak then return (roll/yaw rates reduce, vz reduces)
        """
        ...
        # Vertical velocity: subtle upward component during maximum tilt phases
        # Peaks during carve initiation phases (0.0-0.25 and 0.5-0.75) to counteract sinking
        # Uses absolute value of roll rate as indicator of tilt magnitude
        vz = self.vz_amp * np.abs(np.sin(2 * np.pi * phase))
        
        # No forward/backward motion (vx = 0)
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Rear legs (RL, RR): Anti-phase lateral wave motion with vertical compensation
        ...
        # Rear legs: lateral wave motion with vertical compensation
        if leg_name.startswith('R'):
            ...
            # Add vertical compensation proportional to lateral displacement magnitude
            # This lifts the foot slightly during maximum lateral excursion
            # preventing penetration when combined with roll tilt
            z_compensation = self.lateral_z_compensation * np.abs(lateral_offset) / self.rear_lateral_amplitude
