...
class SKILL_WOBBLE_PLATE_SPIN_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.wobble_amplitude_pitch = 0.12  # radians (~6.9 degrees)
        self.wobble_amplitude_roll = 0.12   # radians (~6.9 degrees)
        ...
        self.foot_adjust_scale_x = 0.6  # pitch (x-direction) compensation
        self.foot_adjust_scale_y = 0.9  # roll (y-direction) compensation - increased for left legs
        
        # Micro-lift parameters for unloaded legs during peak tilts
        self.lift_threshold = 0.035  # radians - lowered from 0.06 for earlier engagement
        self.max_lift_height = 0.03  # meters - maximum lift for unloaded legs
        
        # Constant safety offset for all feet
        self.constant_z_safety_offset = 0.025  # meters - baseline clearance
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        - Add constant safety offset for baseline clearance
        ...
        Z compensation: lift feet appropriately based on tilt geometry
        """
        ...
        if is_front:
            foot[0] += self.foot_adjust_scale_x * pitch_angle
        else:
            foot[0] -= self.foot_adjust_scale_x * pitch_angle
        
        # Roll compensation (y-direction in body frame)
        # Right roll (positive): left legs extend left, right legs extend right
        # Using higher scaling for lateral compensation per expert feedback
        if is_left:
            foot[1] -= self.foot_adjust_scale_y * roll_angle
        else:
            foot[1] += self.foot_adjust_scale_y * roll_angle
        
        # Z compensation: lift feet to maintain ground contact during tilt
        
        # Base Z adjustment for tilt geometry (lift all feet slightly during tilt)
        tilt_magnitude = np.sqrt(pitch_angle**2 + roll_angle**2)
        base_z_lift = 0.5 * self.foot_adjust_scale_x * tilt_magnitude
        foot[2] += base_z_lift
        
        # Micro-lift for unloaded legs during peak tilts
        # Lower threshold (0.035 rad) provides earlier engagement
        ...
        # Pitch-based micro-lift (now full magnitude, not reduced by 0.5)
        ...
        # Add constant safety offset for baseline clearance (per expert feedback: "add slight offset")
        foot[2] += self.constant_z_safety_offset
        
        # Additional small offset for left legs to address asymmetric penetration pattern
        if is_left:
            foot[2] += 0.008  # Extra 8mm clearance for left legs
