...
class SKILL_WOBBLE_PLATE_SPIN_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Wobble parameters
        self.pitch_amplitude = np.deg2rad(10.0)  # ±10° pitch oscillation (reduced for stability)
        ...
        self.vertical_adjustment_scale = 0.050  # Vertical compensation range (increased)
        
        # Base state
        self.t = 0.0
        self.root_pos = np.array([0.0, 0.0, 0.02])  # Small height offset for clearance
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        - When a leg's attachment point is on the downward side of a tilt, 
          the base tilt brings it closer to ground, so the leg must RETRACT (positive z adjustment)
        - When on the upward side, leg must EXTEND (negative z adjustment)
        """
        ...
        # CORRECTED asymmetric vertical compensation
        # Key insight: leg on downward tilt side needs to RETRACT (positive z), not extend
        
        # Pitch contribution to vertical adjustment:
        # Forward pitch (positive) tilts nose down → front attachment points are LOWER in world frame
        # → front legs must RETRACT in body frame (positive adjustment)
        # → rear attachment points are HIGHER → rear legs must EXTEND (negative adjustment)
        if is_front:
            pitch_vertical_contribution = +np.sin(pitch_angle)  # INVERTED: positive = retract when nose down
        else:
            pitch_vertical_contribution = -np.sin(pitch_angle)  # INVERTED: negative = extend when nose down
        
        # Roll contribution to vertical adjustment:
        # Right roll (positive) tilts right side down → right attachment points are LOWER
        # → right legs must RETRACT (positive adjustment)
        # → left attachment points are HIGHER → left legs must EXTEND (negative adjustment)
        if is_left:
            roll_vertical_contribution = -np.sin(roll_angle)  # INVERTED: negative = extend when right side down
        else:
            roll_vertical_contribution = +np.sin(roll_angle)  # INVERTED: positive = retract when right side down
