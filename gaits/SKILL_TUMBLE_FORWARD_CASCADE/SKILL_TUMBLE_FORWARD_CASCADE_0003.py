...
class SKILL_TUMBLE_FORWARD_CASCADE_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        for k, v in initial_foot_positions_body.items():
            pos = v.copy()
            # Ensure initial ground clearance for front legs
            is_front = k.startswith('FL') or k.startswith('FR')
            if is_front and pos[2] > -0.04:
                pos[2] = -0.04  # Ensure minimum upward offset in body frame
            self.base_feet_pos_body[k] = pos
        
        # Relaxed tuck parameters to respect joint limits
        self.tuck_offset_x = 0.02  # Reduced from 0.05
        self.tuck_offset_z = -0.07  # Reduced from -0.10
        self.tuck_radial_scale = 0.68  # Increased from 0.55 for less extreme compression
        
        # Conservative extension parameters
        self.thrust_extension = 0.06
        self.landing_extension_forward = 0.09
        self.landing_extension_down = 0.04
        ...
        if is_front:
            ...
            if phase < 0.2:
                ...
                if progress < 0.4:
                    ...
                    foot[0] = base_pos[0] * (1.0 - 0.32 * retract_smooth) + self.tuck_offset_x * retract_smooth
            ...
            # Phase 0.2-0.7: Tucked during rotation with dynamic depth
                ...
                # Enhanced relief: depth varies from 0.6 to 1.0
                depth_variation = abs(mid_phase - 0.5) / 0.5
                tuck_depth = 0.6 + 0.4 * depth_variation
            ...
            # Phase 0.7-1.0: Extend toward nominal stance
                ...
                tuck_z = base_pos[2] + self.tuck_offset_z * 0.7
        ...
        # REAR LEGS (RL, RR) trajectory
            ...
            # Phase 0.15-0.7: Transition to tucked position
                ...
                if progress < 0.35:
                    ...
                    # Tucked with enhanced dynamic depth relief
                    tuck_progress = (progress - 0.35) / 0.65
                    depth_variation = abs(tuck_progress - 0.5) / 0.5
                    tuck_depth = 0.6 + 0.4 * depth_variation
                    foot[0] = base_pos[0] * self.tuck_radial_scale + self.tuck_offset_x
                    foot[2] = base_pos[2] + self.tuck_offset_z * tuck_depth
