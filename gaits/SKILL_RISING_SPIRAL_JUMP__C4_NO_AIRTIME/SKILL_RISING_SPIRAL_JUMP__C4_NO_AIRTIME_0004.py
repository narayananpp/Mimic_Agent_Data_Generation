...
class SKILL_RISING_SPIRAL_JUMP_MotionGenerator(BaseMotionGenerator):
    ...
    def update_base_motion(self, phase, dt):
        ...
        if phase < 0.2:
            ...
            # Target ~0.14m descent to balance 0.20m foot rise in body frame
            local_phase = phase / 0.2
            vz = -0.65 * self.smooth_step(local_phase)
            yaw_rate = 0.0
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        approximately compensating for base descent to maintain ground contact.
        ...
        if phase < 0.2:
            ...
            # Transition from compressed state back toward nominal then lifting
            if local_phase < 0.7:
                # Still in contact, extending from compressed position
                foot[2] = base_foot[2] + self.compression_depth * (1.0 - smooth_progress / 0.7)
            else:
                # Beginning liftoff
                liftoff_progress = (local_phase - 0.7) / 0.3
                foot[2] = base_foot[2] + 0.04 * self.smooth_step(liftoff_progress)
