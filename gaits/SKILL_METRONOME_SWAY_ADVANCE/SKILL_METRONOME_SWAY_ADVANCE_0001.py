...
class SKILL_METRONOME_SWAY_ADVANCE_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Leg vertical motion parameters (body frame z-offset due to roll)
        self.leg_z_compression_amplitude = 0.06  # Vertical compression/extension range (m)
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        1. Roll-induced vertical geometry changes (z-axis)
        2. Forward surge-induced apparent rearward motion (x-axis)
        
        Left legs (FL, RL):
          - Extend (z increases) when base rolls right (phase ~0.25)
          - Compress (z decreases) when base rolls left (phase ~0.75)
        
        Right legs (FR, RR):
          - Compress (z decreases) when base rolls right (phase ~0.25)
          - Extend (z increases) when base rolls left (phase ~0.75)
        """
        ...
        # Roll angle for this phase (same as in update_base_motion)
        roll_angle = self.roll_amplitude * np.sin(2 * np.pi * phase)
        
        # Vertical offset (z-axis in body frame)
        # Left legs: positive z-offset when roll is positive (rolling right)
        # Right legs: negative z-offset when roll is positive (rolling right)
        if is_left_leg:
            z_offset = self.leg_z_compression_amplitude * np.sin(2 * np.pi * phase)
        elif is_right_leg:
            z_offset = -self.leg_z_compression_amplitude * np.sin(2 * np.pi * phase)
        else:
            z_offset = 0.0
