...
class SKILL_TRIPOD_TILT_WALK_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        for k, v in initial_foot_positions_body.items():
            pos = v.copy()
            # Increased clearance for front legs to handle tilt
    ...
    def get_dynamic_tilt_compensation(self, leg_name, phase):
        ...
        # This provides approximate compensation without explicit integration
        if phase < 0.2:
        ...
        # When tilting left (negative roll), right side goes down
        # When tilting right (positive roll), left side goes down
    ...
    def _compute_fl_trajectory(self, foot_base, phase, leg_name):
        ...
        if phase < 0.2:
            # Stance phase
            progress = phase / 0.2
            foot[0] -= self.step_length * 0.5 * (2 * progress - 1)
            ...
            # Stance contribution
            x_stance = foot_base[0] - self.step_length * 0.5
            z_stance = foot_base[2] + self.get_dynamic_tilt_compensation(leg_name, 0.2)
            
            # Swing contribution
            swing_progress = 0.0
            x_swing = foot_base[0] - self.step_length * 0.5
            z_swing = foot_base[2] + self.step_height * 0.1
            
            foot[0] = x_stance * (1 - blend) + x_swing * blend
            foot[2] = z_stance * (1 - blend) + z_swing * blend
            
        elif phase < 0.8 - blend_window:
            # Main swing phase: full height with smooth arc
            swing_progress = (phase - (0.2 + blend_window)) / (0.6 - 2 * blend_window)
            
            # Forward motion during swing - reduced at peak to avoid workspace limits
            if swing_progress < 0.5:
                x_factor = -0.5 + swing_progress
            else:
                x_factor = -0.5 + swing_progress * 0.8
            foot[0] += self.step_length * x_factor
            
            # Smooth arc trajectory with full step height
            if swing_progress < 0.5:
            ...
            # Swing contribution
            x_swing = foot_base[0] + self.step_length * 0.3
            ...
            foot[0] += self.step_length * 0.5 * (1 - 2 * progress)
    ...
    def _compute_fr_trajectory(self, foot_base, phase, leg_name):
        ...
        High swing during left tilt stance phase. Fixed for ground clearance.
        """
        foot = foot_base.copy()
        ...
        if phase < 0.4 - blend_window:
            # Main swing phase with full step height
            swing_progress = phase / (0.4 - blend_window)
            
            # Forward motion during swing - reduced at peak
            if swing_progress < 0.5:
                x_factor = -0.5 + swing_progress
            else:
                x_factor = -0.5 + swing_progress * 0.8
            ...
            # Smooth transition from stance to swing - CRITICAL FIX
            ...
            # Swing contribution - FULL HEIGHT to prevent penetration
            swing_progress = 0.0
            x_swing = foot_base[0] - self.step_length * 0.5
            z_swing = foot_base[2] + self.step_height * 0.3  # Increased from 0.5 * 0.5
            
            foot[0] = x_stance * (1 - blend) + x_swing * blend
            foot[2] = z_stance * (1 - blend) + z_swing * blend
            
        else:
            # Swing initiation for next cycle - FULL HEIGHT maintained
            swing_progress = (phase - (0.8 + blend_window)) / (0.2 - blend_window)
            foot[0] -= self.step_length * 0.5 * (1 - swing_progress)
            
            # Full step height during swing to avoid ground penetration
    ...
    def _compute_rl_trajectory(self, foot_base, phase, leg_name):
        ...
        foot[0] -= self.step_length * 0.25 * np.sin(progress * np.pi)
