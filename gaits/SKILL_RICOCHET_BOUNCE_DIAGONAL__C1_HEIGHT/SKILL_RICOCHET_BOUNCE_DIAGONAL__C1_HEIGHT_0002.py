...
class SKILL_RICOCHET_BOUNCE_DIAGONAL_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.vz_compression = -0.5  # Reduced downward velocity during compression
        self.vz_launch = 2.2  # Upward velocity during launch
        ...
        # Base state - initialize with proper height clearance
        self.t = 0.0
        self.root_pos = np.array([0.0, 0.0, 0.28])  # Start at safe height
    ...
    def update_base_motion(self, phase, dt):
        ...
        if phase < 0.2:
            ...
            vx = self.vx_cruise * (1.0 - 0.3 * local_phase)
            vy = -0.2 * self.vy_diagonal * local_phase
            # Gentler compression with sinusoidal profile
        ...
        # Phase 0.2-0.4: Right diagonal launch aerial
            ...
            # Parabolic aerial trajectory with smooth transitions
            if local_phase < 0.5:
                vz = self.vz_launch * (1.0 - 1.6 * local_phase)
            else:
                vz = -self.vz_launch * 0.6 * (2.0 * local_phase - 1.0)
        ...
        # Phase 0.8-1.0: Transition preparation with height recovery
        else:
            local_phase = (phase - 0.8) / 0.2
            vx = self.vx_cruise * (0.5 + 0.5 * local_phase)
            vy = -self.vy_diagonal * (1.0 - local_phase) * 0.3
            # Minimal downward motion, allow height stabilization
            vz = self.vz_compression * 0.2 * np.sin(np.pi * local_phase)
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        if phase < 0.2:
            local_phase = phase / 0.2
            if is_left:
                # Primary load-bearing: moderate compression
                foot[2] -= self.compression_depth * 0.4 * (1.0 - np.cos(np.pi * local_phase))
        ...
        # Phase 0.8-1.0: Transition preparation
            ...
            local_phase = (phase - 0.8) / 0.2
            # All feet stabilize with minimal compression
            foot[2] -= self.compression_depth * 0.15 * (1.0 - np.cos(np.pi * local_phase))
