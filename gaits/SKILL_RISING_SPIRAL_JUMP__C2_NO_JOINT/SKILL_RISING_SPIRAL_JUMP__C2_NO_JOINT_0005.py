...
class SKILL_RISING_SPIRAL_JUMP_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Motion parameters - tuned for cyclic consistency and height envelope compliance
        self.compression_depth = 0.12  # Slightly increased for better range of motion
        self.launch_velocity = 0.9     # Reduced to keep base height within [0.1, 0.68]
        self.yaw_rate = 3.0            # Angular velocity around yaw axis (rad/s)
        self.spiral_extension_radius = 0.20  # Radial extension during spiral
        self.spiral_height_offset = 0.06     # Reduced height variation for cleaner landing
        ...
        # Base state - initialize at proper standing height for cyclic consistency
        self.t = 0.0
    ...
    def update_base_motion(self, phase, dt):
        ...
        Height-regulated to stay within [0.1, 0.68] envelope with cyclic consistency.
        """
        ...
        # Phase 2: Explosive launch [0.2, 0.4]
            ...
            # Upward acceleration during launch
        ...
        # Phase 5: Descent and landing [0.8, 1.0]
            ...
            # Height-targeted descent to ensure return to nominal height
            height_error = self.root_pos[2] - self.nominal_standing_height
            target_descent_velocity = -self.launch_velocity * 0.7
            
            # Blend between ballistic descent and height correction
            if local_phase < 0.7:
                vz = target_descent_velocity
            else:
                # Final approach: slow down and target nominal height
                correction_factor = 1.0 - (local_phase - 0.7) / 0.3
                vz = target_descent_velocity * correction_factor - height_error * 2.0
            
            # Decelerate yaw rotation for stable landing
            yaw_rate = self.yaw_rate * (1.0 - smooth_phase)
        
        # Height safety regulation: dampen velocity if approaching limits
        height_margin_top = 0.65
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        """
        Compute foot position in body frame with enforced cyclic consistency.
        Feet must return to exact nominal positions at phase 1.0 to match phase 0.0.
        """
        ...
        if phase < 0.2:
            local_phase = phase / 0.2
            # Small upward shift in body frame as legs compress (kinematic effect)
            smooth_compression = self.smooth_transition(local_phase)
            compression_amount = self.compression_depth * 0.8 * np.sin(np.pi * smooth_compression)
            foot[2] = base_pos[2] + compression_amount
            
        # Phase 2: Explosive launch [0.2, 0.4]
        elif phase < 0.4:
            local_phase = (phase - 0.2) / 0.2
            # Transition from compressed to neutral as legs extend for launch
            smooth_phase = self.smooth_transition(1.0 - local_phase)
            extension = self.compression_depth * 0.8 * smooth_phase
            foot[2] = base_pos[2] + extension
        ...
        # Phase 3: Aerial spiral expansion [0.4, 0.6]
            ...
            # Upward component for spiral aesthetic
        ...
        # Phase 5: Descent and landing [0.8, 1.0]
            ...
            # CRITICAL: Ensure vertical position returns to EXACT nominal by phase 1.0
            if local_phase < 0.85:
                # Retract spiral height offset
                retract_progress = local_phase / 0.85
                smooth_retract_vertical = self.smooth_transition(retract_progress)
                remaining_spiral = self.spiral_height_offset * 0.5 * (1.0 - smooth_retract_vertical)
                foot[2] = base_pos[2] + remaining_spiral
            else:
                # Final landing phase: brief flex then return to exact nominal
                landing_phase = (local_phase - 0.85) / 0.15
                # Create downward flex that returns to zero at phase 1.0
                landing_flex = -self.compression_depth * 0.2 * np.sin(np.pi * landing_phase)
                foot[2] = base_pos[2] + landing_flex
