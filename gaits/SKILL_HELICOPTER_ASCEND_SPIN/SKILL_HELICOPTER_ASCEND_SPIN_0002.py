...
class SKILL_HELICOPTER_ASCEND_SPIN_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Helicopter motion parameters (reduced for kinematic feasibility)
        self.rotor_extension_radius = 0.28  # Reduced from 0.4 to stay within joint limits
        self.rotor_height_offset = -0.12    # Lowered from -0.05 to reduce hip abduction angles
        
        # Vertical motion parameters (adjusted for aerial safety envelope)
        self.initial_height_offset = 0.25   # Start elevated for aerial maneuver clearance
        self.vz_ascent_max = 0.7            # Slightly reduced peak upward velocity
        self.total_ascent_height = 0.5      # Reduced total ascent to control peak height
        ...
        for leg in leg_names:
            ...
            # Front legs slightly more conservative than rear legs
            if leg.startswith('FL') or leg.startswith('FR'):
                leg_radius = self.rotor_extension_radius * 0.95
            else:
                leg_radius = self.rotor_extension_radius
    ...
    def update_base_motion(self, phase, dt):
        ...
        if phase < 0.3:
            ...
            # Final phase: maintain near-zero velocity instead of descending
            # Gentle deceleration to hover state
            progress = (phase - 0.9) / 0.1
            vz = 0.05 * (1.0 - progress)
