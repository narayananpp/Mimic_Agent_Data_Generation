...
class SKILL_HELICOPTER_ASCEND_SPIN_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Helicopter motion parameters
        self.rotor_extension_radius = 0.4  # Horizontal extension distance from body center
        self.rotor_height_offset = -0.05   # Slight negative z in body frame for horizontal blade effect
        
        # Vertical motion parameters
        self.vz_ascent_max = 0.8           # Peak upward velocity
        self.vz_descent = -0.2             # Descent velocity in final phase
        self.total_ascent_height = 0.6     # Approximate integrated height gain
        ...
        for leg in leg_names:
            ...
            # Compute radial direction from body center for horizontal extension
            ...
            # Rotor position: extend radially outward to specified radius
            rotor_x = radial_unit[0] * self.rotor_extension_radius
            rotor_y = radial_unit[1] * self.rotor_extension_radius
    ...
    def update_base_motion(self, phase, dt):
        ...
        - [0.9, 1.0]: Spin down and descent preparation
        """
        
        # Compute vertical velocity based on phase
        if phase < 0.3:
            # Ramp up ascent velocity
            progress = phase / 0.3
            vz = self.vz_ascent_max * progress
        elif phase < 0.6:
            # Sustained ascent at peak velocity
            vz = self.vz_ascent_max
        elif phase < 0.9:
            # Decelerate vertical velocity to zero at peak height
            progress = (phase - 0.6) / 0.3
            vz = self.vz_ascent_max * (1.0 - progress)
        else:
            # Begin descent
            vz = self.vz_descent
        ...
        if phase < 0.3:
            ...
            # Spin down: decelerate toward minimum
            progress = (phase - 0.9) / 0.1
            yaw_rate = self.yaw_rate_max * (1.0 - progress) + self.yaw_rate_min * progress
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        if phase < 0.3:
            ...
            progress = phase / 0.3
            # Use smooth ease-out for more natural motion
