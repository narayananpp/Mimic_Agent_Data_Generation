...
class SKILL_PROPELLER_SPIN_ADVANCE_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Circular rotation parameters - reduced for kinematic feasibility
        self.circle_radius = 0.06  # Reduced radius to respect joint limits
        self.circle_center_offset_x = 0.0  # Forward offset of circle center from base foot position
        self.circle_center_offset_z = 0.08  # Raised to prevent ground penetration
        ...
        self.pitch_amplitude = 0.10  # Reduced pitch amplitude for smoother motion
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Circle parametrization (corrected):
        - angle = 2π * leg_phase
        - x_offset = radius * sin(angle)  (forward-backward motion)
        - z_offset = radius * cos(angle)  (up-down motion, positive cos at phase 0 = top)
        
        At leg_phase = 0.0: angle = 0, foot at top (x = 0, z = +radius)
        At leg_phase = 0.25: angle = π/2, foot at forward mid (x = +radius, z = 0)
        At leg_phase = 0.5: angle = π, foot at bottom (x = 0, z = -radius)
        ...
        # Circular trajectory offsets (corrected sign for z to place top at phase 0)
