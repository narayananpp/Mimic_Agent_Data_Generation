...
class SKILL_RISING_SPIRAL_JUMP_MotionGenerator(BaseMotionGenerator):
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Key strategy:
        - Minimal z-offsets during ascent/spiral extension (0.4-0.6)
        - Progressive foot tucking during descent (0.6-0.85) to maintain clearance
        - Smooth landing transition (0.85-0.95) from tucked to ground contact
        """
        ...
        if phase < 0.2:
            ...
            # Early aerial spiral phase: sequential radial extension with minimal z offset
            ...
            # Transition to descent: begin tucking feet upward
            progress = (phase - 0.6) / 0.05
            tuck_curve = np.sin(progress * np.pi / 2)
            
            # Maintain radial extension, increase z-offset for tucking
            foot[0] = base_pos[0] * (1.08 + (self.max_radial_extension_x - 1.08) * leg_factor)
            foot[1] = base_pos[1] * (1.08 + (self.max_radial_extension_y - 1.08) * leg_factor)
            foot[2] = base_pos[2] + 0.01 + 0.02 * tuck_curve  # tuck from +0.01 to +0.03
            
        elif phase < 0.75:
            # Active descent: maintain tucked position, begin radial retraction
            progress = (phase - 0.65) / 0.10
            retract_curve = np.sin(progress * np.pi / 2)
            
            # Begin retracting radially
            current_radial_x = (1.08 + (self.max_radial_extension_x - 1.08) * leg_factor) * (1.0 - 0.3 * retract_curve) + 1.0 * 0.3 * retract_curve
            current_radial_y = (1.08 + (self.max_radial_extension_y - 1.08) * leg_factor) * (1.0 - 0.3 * retract_curve) + 1.0 * 0.3 * retract_curve
            
            foot[0] = base_pos[0] * current_radial_x
            foot[1] = base_pos[1] * current_radial_y
            foot[2] = base_pos[2] + 0.03  # maintain tuck
            
        elif phase < 0.85:
            # Deep descent: increase tuck, continue retraction
            progress = (phase - 0.75) / 0.10
            retract_curve = 0.3 + 0.5 * np.sin(progress * np.pi / 2)  # continue from 0.3 to 0.8
            ...
            foot[0] = base_pos[0] * current_radial_x
            foot[1] = base_pos[1] * current_radial_y
            foot[2] = base_pos[2] + 0.03 + 0.01 * tuck_increase  # increase tuck to +0.04
            
        elif phase < 0.95:
            # Landing preparation: smooth descent from tucked to ground contact
            progress = (phase - 0.85) / 0.10
            landing_curve = np.sin(progress * np.pi / 2)
            
            # Complete radial retraction to nominal
            foot[0] = base_pos[0]
            foot[1] = base_pos[1]
            # Smooth descent from +0.04 to nominal
            foot[2] = base_pos[2] + 0.04 * (1.0 - landing_curve)
