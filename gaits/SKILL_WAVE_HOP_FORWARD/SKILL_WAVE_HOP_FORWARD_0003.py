...
class SKILL_WAVE_HOP_FORWARD_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.compression_depth = 0.015  # Reduced leg compression during loading
        
        # Base motion parameters - rebalanced for height envelope
        self.forward_velocity = 0.6  # Forward velocity during thrust
        self.vertical_velocity_thrust = 0.45  # Reduced upward velocity during thrust
        self.vertical_velocity_apex = 0.2  # Reduced upward velocity at liftoff
    ...
    def update_base_motion(self, phase, dt):
        ...
        Vertical velocity profile rebalanced for height envelope compliance.
        """
        vx = 0.0
        vy = 0.0
        vz = 0.0
        ...
        if phase < 0.15:
            ...
            # Reduced initial upward velocity with smooth decay
            vz = self.vertical_velocity_thrust * (1.0 - progress * 0.4)
            ...
            # Reduced apex velocity with stronger decay
            vz = self.vertical_velocity_apex * (1.0 - progress * 0.7)
            pitch_rate = self.pitch_rate_level * progress
            
        elif phase < 0.5:
            # Flight apex: aggressive transition from ascent to descent
            progress = (phase - 0.3) / 0.2
            vx = self.forward_velocity * 0.7 * (1.0 - progress * 0.3)
            # Clear inflection point with decisive transition to descent
            apex_vz_start = self.vertical_velocity_apex * 0.3
            apex_vz_end = -0.35
            vz = apex_vz_start + (apex_vz_end - apex_vz_start) * progress
            pitch_rate = 0.0
            
        elif phase < 0.65:
            # Rear landing: increased downward velocity for balance
            progress = (phase - 0.5) / 0.15
            vx = self.forward_velocity * 0.4 * (1.0 - progress * 0.5)
            # Increased descent velocity magnitude
            vz = -0.35 - progress * 0.15
            pitch_rate = self.pitch_rate_landing * (1.0 - progress * 0.5)
            
        elif phase < 0.8:
            # Front landing: continued controlled descent
            progress = (phase - 0.65) / 0.15
            vx = self.forward_velocity * 0.2 * (1.0 - progress)
            # Maintained descent velocity for height recovery
            vz = -0.5 + progress * 0.35
            pitch_rate = self.pitch_rate_correct * progress
            
        else:
            # Compression and loading: minimal settling velocity
            progress = (phase - 0.8) / 0.2
            vx = 0.05 * (1.0 - progress)
            # Small settling velocity with smooth decay
            vz = -0.15 * (1.0 - progress * 0.9)
            pitch_rate = 0.0
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Compression is applied only when base is stable to prevent ground penetration.
        """
        ...
        if is_rear:
            ...
            if phase < 0.15:
                ...
                # Landing phase: contact with minimal compression
                progress = (phase - 0.5) / 0.15
                # Minimal landing compression to prevent penetration
                compress_curve = np.sin(progress * np.pi / 2)
                foot[0] += self.forward_reach * 0.5
                foot[2] -= self.compression_depth * 0.2 * compress_curve
                
            else:
                # Stance phase: gradual compression only in late phase
                progress = (phase - 0.65) / 0.35
                if progress < 0.6:
                    # Minimal compression during front landing
                    foot[0] += self.forward_reach * 0.5
                    foot[2] -= self.compression_depth * 0.2
                else:
                    # Final compression for energy loading
                    compress_progress = (progress - 0.6) / 0.4
                    foot[0] += self.forward_reach * 0.5
                    foot[2] -= self.compression_depth * (0.2 + 0.5 * compress_progress)
        
        elif is_front:
            # Front legs: stance [0.0, 0.15], liftoff [0.15, 0.3], flight [0.3, 0.65], land [0.65, 0.8], stance [0.8, 1.0]
            if phase < 0.15:
                # Stance phase: providing thrust with minimal extension
                progress = phase / 0.15
                # Reduced extension during thrust
                foot[2] += self.compression_depth * 0.15 * progress
                ...
                progress = (phase - 0.15) / 0.15
                lift_curve = np.sin(progress * np.pi / 2)
                foot[2] += self.compression_depth * 0.15 + self.tuck_height * lift_curve
                ...
                compress_curve = np.sin(progress * np.pi / 2)
                foot[0] += self.forward_reach * 0.5
                # Minimal compression on landing
                foot[2] -= self.compression_depth * 0.3 * compress_curve
                
            else:
                # Stance phase: gradual final compression
                progress = (phase - 0.8) / 0.2
                foot[0] += self.forward_reach * 0.5
                # Progressive compression to maximum
                foot[2] -= self.compression_depth * (0.3 + 0.7 * progress)
