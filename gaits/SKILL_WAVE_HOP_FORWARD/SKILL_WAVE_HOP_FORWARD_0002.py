...
class SKILL_WAVE_HOP_FORWARD_MotionGenerator(BaseMotionGenerator):
    ...
    def update_base_motion(self, phase, dt):
        ...
        Reduced downward velocities during landing to prevent ground penetration.
        """
        vx = 0.0
        vy = 0.0
        vz = 0.0
        ...
        if phase < 0.15:
            ...
            # Flight apex: ballistic trajectory with smooth transition to descent
            progress = (phase - 0.3) / 0.2
            vx = self.forward_velocity * 0.7 * (1.0 - progress * 0.3)
            # Smooth transition from ascent to descent through apex
            vz = self.vertical_velocity_apex * 0.5 * (1.0 - progress) - 0.15 * progress
            pitch_rate = 0.0
            
        elif phase < 0.65:
            # Rear landing: minimal downward velocity, nose down
            progress = (phase - 0.5) / 0.15
            vx = self.forward_velocity * 0.4 * (1.0 - progress * 0.5)
            # Reduced downward velocity during landing
            vz = -0.15 - progress * 0.1
            pitch_rate = self.pitch_rate_landing * (1.0 - progress * 0.5)
            
        elif phase < 0.8:
            # Front landing: complete deceleration, minimal descent
            progress = (phase - 0.65) / 0.15
            vx = self.forward_velocity * 0.2 * (1.0 - progress)
            # Minimal downward velocity during front landing
            vz = -0.2 * (1.0 - progress * 0.6)
            pitch_rate = self.pitch_rate_correct * progress
            
        else:
            # Compression and loading: near-zero vertical velocity
            progress = (phase - 0.8) / 0.2
            vx = 0.05 * (1.0 - progress)
            # Very minimal settling velocity
            vz = -0.05 * (1.0 - progress * 0.8)
            pitch_rate = 0.0
