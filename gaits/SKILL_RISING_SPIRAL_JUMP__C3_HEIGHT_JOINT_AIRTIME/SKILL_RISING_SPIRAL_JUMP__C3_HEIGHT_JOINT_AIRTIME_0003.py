...
class SKILL_RISING_SPIRAL_JUMP_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Jump parameters - FURTHER REDUCED to respect height envelope [0.1, 0.68]
        self.compression_depth = 0.12  # Increased from 0.10 to lower starting point
        self.launch_vz = 0.72  # Reduced from 1.2 to 0.72 to constrain peak height
        
        # Leg compression during crouch
        self.leg_compression_z = 0.10  # Moderate compression
        
        # Spiral extension parameters - kept moderate
        self.spiral_extension_radius = 0.22
        self.spiral_extension_z = 0.12
    ...
    def update_base_motion(self, phase, dt):
        ...
        # Aerial spiral extension [0.4, 0.6]: upward velocity decelerating aggressively
        elif phase < 0.6:
            local_phase = (phase - 0.4) / 0.2
            # Decelerate upward velocity more aggressively to limit peak height
            vz = self.launch_vz * np.cos(local_phase * np.pi / 2) * (1.0 - local_phase * 0.3)
            yaw_rate = self.yaw_rate_max  # Constant yaw rate
        
        # Peak extension [0.6, 0.8]: transition to downward velocity more aggressively
        elif phase < 0.8:
            local_phase = (phase - 0.6) / 0.2
            # Accelerate downward more aggressively to arrest upward momentum
            vz = -self.launch_vz * np.sin(local_phase * np.pi / 2) * 1.05
        ...
        # Descent and landing [0.8, 1.0]: downward velocity, yaw stops
            ...
            vz = -self.launch_vz * 1.05 * (1.0 - local_phase * 0.6)
