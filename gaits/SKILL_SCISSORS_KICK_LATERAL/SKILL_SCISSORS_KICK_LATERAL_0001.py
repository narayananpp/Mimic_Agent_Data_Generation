...
class SKILL_SCISSORS_KICK_LATERAL_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Scissor kick parameters
        self.scissor_amplitude = 0.12  # Vertical extension distance for scissor motion
        self.lateral_velocity_magnitude = 0.3  # Lateral velocity during scissor kicks
        self.roll_compensation = 0.15  # Roll rate to compensate body tilt during kicks
    ...
    def update_base_motion(self, phase, dt):
        ...
        if 0.0 <= phase < 0.2:
            ...
            vz = 0.05 * np.sin(2 * np.pi * progress)  # Slight vertical oscillation
            roll_rate = -self.roll_compensation * np.sin(np.pi * progress)
            
        elif 0.2 <= phase < 0.4:
            # Right return: decaying rightward momentum
            progress = (phase - 0.2) / 0.2
            vy = self.lateral_velocity_magnitude * 0.5 * (1.0 - progress)
            roll_rate = -self.roll_compensation * 0.3 * (1.0 - progress)
            ...
            # Left return: decaying leftward momentum
            progress = (phase - 0.6) / 0.2
            vy = -self.lateral_velocity_magnitude * 0.5 * (1.0 - progress)
            roll_rate = self.roll_compensation * 0.3 * (1.0 - progress)
            
        else:  # 0.8 <= phase < 1.0
            # Settle phase: approaching zero velocity
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        RL: neutral [0.0-0.4], downward push [0.4-0.6], return/neutral [0.6-1.0]
        RR: downward push [0.0-0.2], return/neutral [0.2-1.0]
        """
        foot = self.base_feet_pos_body[leg_name].copy()
        
        if leg_name.startswith('FR'):
            if 0.0 <= phase < 0.2:
                # Upward scissor extension
                progress = phase / 0.2
                angle = np.pi * progress
                foot[2] += self.scissor_amplitude * np.sin(angle)
            elif 0.2 <= phase < 0.4:
                # Return to neutral
                progress = (phase - 0.2) / 0.2
                angle = np.pi * (1.0 - progress)
                foot[2] += self.scissor_amplitude * np.sin(angle) * 0.3 * (1.0 - progress)
            # else: remain at neutral
            
        elif leg_name.startswith('RR'):
            if 0.0 <= phase < 0.2:
                # Downward scissor extension (negative Z)
                progress = phase / 0.2
                angle = np.pi * progress
                foot[2] -= self.scissor_amplitude * np.sin(angle)
            elif 0.2 <= phase < 0.4:
