...
class SKILL_RICOCHET_BOUNCE_DIAGONAL_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.compression_depth = 0.07  # Vertical compression during stance
        self.launch_height = 0.25      # Maximum aerial height
        self.step_height = 0.07        # Reduced from 0.10 to limit knee flexion
        ...
        self.launch_velocity = 2.2     # Increased to prevent base sinking
        self.compression_velocity = -0.55  # Reduced to limit downward integration
    ...
    def update_base_motion(self, phase, dt):
        ...
        Includes initialization height boost to prevent startup ground penetration.
        """
        vx = 0.0
        vy = 0.0
        vz = 0.0
        ...
        # Initialization height boost to prevent startup ground penetration
        init_boost = 0.0
        if phase < 0.15:
            init_boost = 0.4 * (1.0 - phase / 0.15)
        ...
        if phase < 0.2:
            ...
            vz = self.compression_velocity * np.sin(np.pi * sub_phase) + init_boost
        ...
        # Phase 0.8-1.0: Landing preparation - stabilize height
        else:
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Smoothed transitions with gradual compression onset and reduced retraction to prevent joint violations.
        """
        ...
        if phase < 0.2:
            ...
            # Gradual compression onset to prevent startup violations
            ramp = min(1.0, sub_phase / 0.25)
            
            # Smooth sinusoidal compression
            foot[2] += self.compression_depth * compression_factor * np.sin(np.pi * sub_phase) * ramp
        ...
        # Phase 0.2-0.4: Right-diagonal launch and aerial
            ...
            # Extension phase (0.0-0.30)
            if sub_phase < 0.30:
                # Gradual extension using cosine smoothing
                extension = 0.5 * (1.0 - np.cos(np.pi * sub_phase / 0.30))
                foot[2] += self.compression_depth * (1.0 - extension) * (1.0 if is_left_leg else 0.7)
            
            # Neutral hold (0.30-0.35) - legs fully extended, no additional motion
            elif sub_phase < 0.35:
                pass  # Maintain extended position
            
            # Aerial retraction phase (0.35-1.0)
            else:
                retract = (sub_phase - 0.35) / 0.65
                # Reduced retraction magnitude and scaling for knee joint safety
