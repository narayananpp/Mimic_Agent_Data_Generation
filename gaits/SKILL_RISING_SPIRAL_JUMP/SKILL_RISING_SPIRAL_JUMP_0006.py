...
class SKILL_RISING_SPIRAL_JUMP_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Motion parameters - ADJUSTED FOR HEIGHT ENVELOPE SAFETY
        self.compression_height = 0.06  # Reduced vertical compression distance
        self.launch_vz = 0.65  # REDUCED upward velocity to stay within height envelope
        self.yaw_rate_max = 3.0  # Yaw angular velocity (rad/s)
        self.spiral_extension = 0.12  # Reduced radial extension for stability
    ...
    def update_base_motion(self, phase, dt):
        ...
        # Aerial rising [0.4, 0.6]
            ...
            # Upward velocity decreasing with quadratic decay (simulating gravity)
            progress = (phase - 0.4) / 0.2
            vz = self.launch_vz * (1.0 - progress * progress)
            # Constant yaw rotation
            yaw_rate = self.yaw_rate_max
        
        # Aerial peak [0.6, 0.8]
        elif phase < 0.8:
            # Transition from upward to downward with quadratic profile
