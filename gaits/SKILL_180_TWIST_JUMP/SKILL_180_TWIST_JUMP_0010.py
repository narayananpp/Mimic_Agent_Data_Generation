...
class SKILL_180_TWIST_JUMP_MotionGenerator(BaseMotionGenerator):
    ...
    Phase breakdown:
      0.00-0.03: Stabilization (neutral stance, allow phase wrap settling)
      0.03-0.18: Compression (crouch down, initiate yaw)
      ...
      0.92-1.00: Landing absorption (contact, stabilize, return to neutral with upward recovery)
    """
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Motion parameters - tuned for phase wrap continuity
        self.compression_depth = 0.07  # Reduced compression to minimize phase wrap impact (m)
        self.tuck_height = 0.09  # Moderate tuck during aerial phase (m)
        self.tuck_inward = 0.035  # Minimal inward tuck (m)
        self.landing_extension = 0.065  # Extension for ground clearance (m)
    ...
    def update_base_motion(self, phase, dt):
        """
        Update base motion with upward recovery during landing to restore initial base height.
        """
        vx = 0.0
        vy = 0.0
        vz = 0.0
        ...
        # Phase 0.0-0.03: Stabilization (neutral stance)
        if phase < 0.03:
            local_phase = phase / 0.03
            # Minimal motion, allow settling from phase wrap
            vz = 0.0
            yaw_rate = 0.0

        # Phase 0.03-0.18: Compression
        elif phase < 0.18:
            local_phase = (phase - 0.03) / 0.15
            # Smooth downward motion during compression
            vz = -0.30 * np.sin(np.pi * local_phase)
        ...
        # Phase 0.68-0.92: Pre-landing
        elif phase < 0.92:
            local_phase = (phase - 0.68) / 0.24
            # Controlled downward velocity
            vz = -self.launch_vz * 0.32 * (0.35 + 0.65 * local_phase * (1.0 - 0.35 * local_phase))
            # Yaw rate decreasing
            yaw_rate = self.peak_yaw_rate * ((1.0 - local_phase) ** 2.2)

        # Phase 0.92-1.0: Landing absorption WITH UPWARD RECOVERY
        else:
            local_phase = (phase - 0.92) / 0.08
            
            # Three-stage landing: downward damping -> pause -> upward recovery
            if local_phase < 0.4:
                # Early landing: downward velocity damping
                decay_factor = (1.0 - (local_phase / 0.4)) ** 3.0
                vz = -self.launch_vz * 0.08 * decay_factor
            elif local_phase < 0.6:
                # Mid landing: brief pause near zero velocity
                vz = 0.0
            else:
                # Late landing: upward recovery to restore base height
                recovery_phase = (local_phase - 0.6) / 0.4
                # Smooth ramp up then maintain
                vz = 0.18 * (0.5 * (1.0 - np.cos(np.pi * min(recovery_phase * 1.5, 1.0))))
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        """
        Compute foot position in body frame with stabilization phase and reduced compression.
        """
        ...
        # Phase 0.0-0.03: Stabilization (neutral stance)
        if phase < 0.03:
            # Feet remain at base position, no offsets
            pass
        ...
        # Phase 0.03-0.18: Compression
            ...
            foot[0] += inward_x_factor * 0.015 * compression_progress
            foot[1] += inward_y_factor * 0.015 * compression_progress
        ...
        # Phase 0.18-0.38: Launch
            ...
            foot[0] += inward_x_factor * 0.015 * extension_progress
            foot[1] += inward_y_factor * 0.015 * extension_progress
        ...
        # Phase 0.68-0.92: Pre-landing (blended untuck and extend)
            ...
            # Smooth untuck progression
            untuck_progress = 0.5 * (1.0 + np.cos(np.pi * (1.0 - local_phase)))
            
            # Smooth extension progression
        ...
        # Phase 0.92-1.0: Landing absorption with delayed return to neutral
            ...
            # Maintain extension through early landing, then return to neutral
            if local_phase < 0.35:
                extension_remaining = self.landing_extension
            else:
                # Gradually return to neutral for phase continuity
                retract_progress = (local_phase - 0.35) / 0.65
