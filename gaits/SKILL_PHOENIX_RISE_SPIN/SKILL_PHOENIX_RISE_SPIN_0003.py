...
class SKILL_PHOENIX_RISE_SPIN_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.max_extension_factor = 1.15  # Radial extension multiplier at peak (reduced for joint safety)
        
        # Base motion parameters
        self.crouch_depth = 0.06  # Base descends during crouch (meters, reduced for safety)
        self.rise_height = 0.18  # Total vertical rise from crouch (meters)
        ...
        # Initialize base height from foot positions - assume feet start at ground level
        # Compute initial base height as average of foot z-positions (negative, as feet are below base)
        avg_foot_z = np.mean([pos[2] for pos in self.base_feet_pos_body.values()])
        self.initial_base_height = -avg_foot_z  # Convert to base height above ground
        if self.initial_base_height < 0.2:
            self.initial_base_height = 0.28  # Fallback to typical quadruped standing height
        
        # Store ground level for each foot in world frame (assumed zero initially)
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        if phase < 0.2:
            # Crouch phase - legs compressed radially toward body (moderate compression)
            radial_factor = 0.85
        elif phase < 0.4:
            # Rise initiation - begin extending
            sub_phase = (phase - 0.2) / 0.2
            sub_smoothed = 0.5 * (1.0 - np.cos(sub_phase * np.pi))
            radial_factor = 0.85 + 0.10 * sub_smoothed  # 0.85 -> 0.95
        elif phase < 0.6:
            # Mid-rise spread
            sub_phase = (phase - 0.4) / 0.2
            sub_smoothed = 0.5 * (1.0 - np.cos(sub_phase * np.pi))
            radial_factor = 0.95 + 0.10 * sub_smoothed  # 0.95 -> 1.05
        elif phase < 0.8:
            # Peak ascent
            sub_phase = (phase - 0.6) / 0.2
            sub_smoothed = 0.5 * (1.0 - np.cos(sub_phase * np.pi))
            radial_factor = 1.05 + 0.07 * sub_smoothed  # 1.05 -> 1.12
        else:
            # Peak hold and completion
            sub_phase = (phase - 0.8) / 0.2
            sub_smoothed = 0.5 * (1.0 - np.cos(sub_phase * np.pi))
            radial_factor = 1.12 + 0.03 * sub_smoothed  # 1.12 -> 1.15
        ...
        # Compute current base height trajectory to maintain ground contact
        if phase < 0.2:
            crouch_progress = phase / 0.2
            crouch_smoothed = 0.5 * (1.0 - np.cos(crouch_progress * np.pi))
        ...
        # Adjust foot Z in body frame to compensate for base height changes
        # As base rises, foot must extend further downward in body frame to maintain ground contact
        base_height_delta = current_base_height - self.initial_base_height
        foot_z = base_z - base_height_delta
