...
class SKILL_PHOENIX_RISE_SPIN_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.max_extension_factor = 1.35  # Radial extension multiplier at peak (reduced from 1.8)
        
        # Base motion parameters
        self.crouch_depth = 0.08  # Base descends during crouch (meters)
        self.rise_height = 0.20  # Total vertical rise from crouch (meters)
    ...
    def update_base_motion(self, phase, dt):
        ...
        # Target base height based on phase
        if phase < 0.2:
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Feet remain at ground contact height (no upward reach).
        """
        ...
        # Compute radial extension factor based on phase (smooth progression)
        if phase < 0.2:
            ...
            radial_factor = 0.75 + 0.20 * sub_smoothed  # 0.75 -> 0.95
        elif phase < 0.6:
            # Mid-rise spread
            sub_phase = (phase - 0.4) / 0.2
            sub_smoothed = 0.5 * (1.0 - np.cos(sub_phase * np.pi))
            radial_factor = 0.95 + 0.20 * sub_smoothed  # 0.95 -> 1.15
        elif phase < 0.8:
            # Peak ascent
            sub_phase = (phase - 0.6) / 0.2
            sub_smoothed = 0.5 * (1.0 - np.cos(sub_phase * np.pi))
            radial_factor = 1.15 + 0.15 * sub_smoothed  # 1.15 -> 1.30
        else:
            # Peak hold and completion
            sub_phase = (phase - 0.8) / 0.2
            sub_smoothed = 0.5 * (1.0 - np.cos(sub_phase * np.pi))
            radial_factor = 1.30 + 0.05 * sub_smoothed  # 1.30 -> 1.35
        
        # Compute new foot position in body frame (radial extension only)
        new_radius = base_radius * radial_factor
        foot_x = new_radius * np.cos(angle)
        foot_y = new_radius * np.sin(angle)
        foot_z = base_z  # Keep original z in body frame to maintain ground contact
