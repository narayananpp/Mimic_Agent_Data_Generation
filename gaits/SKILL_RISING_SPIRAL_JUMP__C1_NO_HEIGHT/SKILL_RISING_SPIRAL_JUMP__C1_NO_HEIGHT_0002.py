...
class SKILL_RISING_SPIRAL_JUMP_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.crouch_depth = 0.06  # Reduced from 0.12 to minimize joint stress
        self.max_radial_extension = 0.13  # Reduced from 0.25 to respect kinematic limits
        self.aerial_lift_height = 0.02  # Reduced from 0.05, only during early aerial
        
        # Launch parameters
        self.launch_vz_max = 2.2  # Slightly reduced for smoother motion
        
        # Yaw rotation parameters
        self.yaw_rate_max = 2.8  # Slightly reduced for stability
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        if phase < 0.2:
            progress = phase / 0.2
            # CRITICAL FIX: Move feet DOWN (negative z) during crouch to keep them grounded
            # Use smooth sinusoidal profile
            crouch_factor = np.sin(np.pi * progress / 2.0)
            foot[2] -= self.crouch_depth * crouch_factor  # Changed from += to -=
        ...
        # Phase 3: Aerial - spiral expansion [0.4, 0.8]
            ...
            if phase < spiral_start:
                ...
                # Apply lift only during early extension phase
                lift = self.aerial_lift_height * smooth_progress
            else:
                # Maximum extension maintained
                radial_extension = self.max_radial_extension
                # Minimal lift at peak
                lift = self.aerial_lift_height * 0.5
        ...
        # Phase 4: Descent and landing [0.8, 1.0]
            ...
            # Smoothly lower feet to ground plane for landing
            # At phase 0.8: foot is at aerial_lift_height * 0.5
            # At phase 1.0: foot should be at nominal or slightly below for firm contact
            start_height = self.aerial_lift_height * 0.5
            end_height = 0.0
