...
class SKILL_RICOCHET_BOUNCE_DIAGONAL_MotionGenerator(BaseMotionGenerator):
    ...
    Final correction with increased stance clearance and fixed landing transition.
    """
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.vertical_velocity_compress = -0.24  # REDUCED from -0.28 to minimize base descent
        
        # Yaw parameters
        self.yaw_rate_compress = 1.6
        
        # Leg motion parameters - FINAL TUNING
        self.compression_depth = 0.06
        self.retraction_height = 0.12
        self.extension_distance = 0.06
        self.stance_clearance = 0.22  # INCREASED from 0.18 for multi-cycle margin
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        """
        Compute foot position in body frame with maximum clearance margins.
        
        Key corrections:
        - stance_clearance = 0.22m provides margin for compression + base descent + settling
        - retraction_height = 0.12m with full multiplier ensures swing clearance
        - landing phase uses gradual clearance buildup (not decompression) for smooth transition
        """
        ...
        if phase < 0.2:
            ...
            if is_left_leg:
                # Stance: maximum clearance margin to prevent penetration
                foot[2] = foot[2] + self.stance_clearance - self.compression_depth * compression_progress
            else:
                # Swing: full retraction height for clearance
        ...
        # Phase 0.4-0.6: Right compression (FR, RR stance)
            ...
            if is_right_leg:
                # Stance: maximum clearance margin
                foot[2] = foot[2] + self.stance_clearance - self.compression_depth * compression_progress
            else:
        ...
        # Phase 0.8-1.0: Landing prep (FL, RL landing)
            ...
            if is_left_leg:
                # CORRECTED: Gradual clearance buildup from aerial to full stance clearance
                # Not decompression - legs are transitioning from aerial, not compressed state
                # At phase 0.8: start with moderate clearance (30% of full)
                # At phase 1.0: reach full stance clearance for next cycle compression
                clearance_factor = 0.3 + 0.7 * landing_progress
