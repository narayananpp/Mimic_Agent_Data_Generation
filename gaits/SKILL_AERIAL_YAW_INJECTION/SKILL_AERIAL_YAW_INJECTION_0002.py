...
class SKILL_AERIAL_YAW_INJECTION_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.peak_height = 0.35  # Maximum altitude during flight
        self.yaw_rate_max = 2.5  # Max yaw rate during injection (rad/s)
        
        # Limb motion parameters
        self.retract_distance = 0.12  # How far limbs pull inward vertically
        self.retract_xy_factor = 0.7  # XY retraction factor (closer to body)
        self.sweep_amplitude = 0.08  # Amplitude of sweep motion
        self.landing_extension = 0.08  # Leg extension for landing prep
    ...
    def _compute_vertical_trajectory(self, phase):
        ...
        Ensures z returns to 0 at phase 1.0 with continuous derivatives.
        """
        if phase < 0.2:
            ...
            z_vel = (0.08 * (6 * progress - 6 * progress**2)) / (0.2 / self.freq)
            ...
            # Derivative: dz/dphase * dphase/dt = dz/dphase * freq
            ...
            # At phase 0.8, z should match end of aerial arc
            z_start = 0.08 + (self.peak_height - 0.08) * np.sin(0.6 / 0.6 * np.pi)
            z_pos = z_start * (1.0 - smoothstep)
            
            # Add impact absorption after phase 0.9
            if progress > 0.5:
                absorption_progress = (progress - 0.5) / 0.5
                z_pos -= 0.03 * (1.0 - np.cos(np.pi * absorption_progress))
            
            # Velocity derivative
            z_vel = -z_start * (6 * progress - 6 * progress**2) / (0.2 / self.freq)
            if progress > 0.5:
                absorption_progress = (progress - 0.5) / 0.5
                z_vel -= 0.03 * np.pi * np.sin(np.pi * absorption_progress) / (0.1 / self.freq)
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        if phase < 0.2:
            ...
            sweep_x = self.sweep_amplitude * np.sin(sweep_angle)
            sweep_y = self.sweep_amplitude * (1.0 - np.cos(sweep_angle)) * np.sign(base_y)
            ...
            # Decay sweep back to retracted position
            ...
            if progress > 0.6:
                ...
                foot[2] -= 0.04 * np.sin(np.pi * absorption_progress)
