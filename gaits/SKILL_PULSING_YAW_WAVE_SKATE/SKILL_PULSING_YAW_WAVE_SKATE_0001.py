...
class SKILL_PULSING_YAW_WAVE_SKATE_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.extension_amplitude = 0.12  # radial extension range (meters)
        self.extension_vertical_bias = -0.06  # downward bias during extension
        ...
        self.vz_amplitude = 0.4  # vertical velocity oscillation amplitude (m/s)
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        All feet remain in contact (z <= 0 in body frame relative to base foot position).
        
        Extension pattern:
        - Phase [0.0, 0.5]: extending (legs lengthen radially outward and downward)
        - Phase [0.5, 1.0]: contracting (legs shorten radially inward and upward)
        """
        ...
        # Vertical offset: legs extend downward during extension (more negative z)
        # Use absolute value of extension_factor to ensure downward bias during both extension peaks
        vertical_offset = self.extension_vertical_bias * np.abs(extension_factor)
        
        # Compute final foot position
        foot = foot_base.copy()
        foot[0] += radial_offset_xy[0]
        foot[1] += radial_offset_xy[1]
        foot[2] += vertical_offset  # more negative during extension (pressing down)
