...
class SKILL_PINWHEEL_LATERAL_SPIN_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Pinwheel rotation parameters - reduced radius for continuous ground contact
        self.rotation_radius = 0.18  # Reduced from 0.25 to ensure reachability
        self.ground_height = -0.3  # Nominal ground contact height in body frame
        
        # Stagger phases slightly to ensure at least some feet maintain contact
        # while preserving visual synchronization of the pinwheel pattern
        self.phase_offsets = {
            leg_names[0]: 0.0,
            leg_names[1]: 0.05,
            leg_names[2]: 0.025,
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        # Adaptive vertical position to maintain ground contact throughout rotation
        # The vertical component must compensate for geometric constraints of circular motion
        # Base ground height with enhanced vertical modulation
        foot_z = self.ground_height
        
        # Enhanced vertical modulation to ensure ground contact at all rotation angles
        # Amplitude increased significantly to compensate for circular trajectory geometry
        # Phase-aligned to provide maximum downward reach when horizontal circular path
        # would geometrically lift the foot highest
        vertical_amplitude = 0.12
        
        # Use combination of fundamental and harmonic frequencies for smooth vertical motion
        # that maintains ground contact while following circular horizontal trajectory
        vertical_offset = vertical_amplitude * (
            np.sin(2 * np.pi * leg_phase) * 0.6 +
            np.sin(4 * np.pi * leg_phase) * 0.25 +
            np.sin(6 * np.pi * leg_phase) * 0.15
        )
        
        foot_z += vertical_offset
        
        # Additional ground contact enforcement: clamp to ensure foot doesn't go above safe threshold
        # This acts as safety mechanism while allowing natural vertical variation
        max_height = self.ground_height + 0.05
        foot_z = min(foot_z, max_height)
