...
class SKILL_WAVE_TOE_SURF_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Base foot positions (toe contact) - use provided positions as ground-level reference
        self.base_feet_pos_body = {}
        for k, v in initial_foot_positions_body.items():
            pos = v.copy()
            # Trust provided positions as actual ground contact points
        ...
        self.toe_height_variation = 0.012  # Vertical variation for contact modulation (upward only)
        ...
        self.roll_rate_amplitude = 0.18  # Roll rate amplitude (rad/s)
    ...
    def update_base_motion(self, phase, dt):
        ...
        - Roll rate: sinusoidal modulation with restoring component to prevent unbounded accumulation
        """
        ...
        # Roll rate includes wave pattern and restoring component
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        All feet maintain toe contact throughout with asymmetric vertical variations:
        - Loading phase (inward motion): firm ground contact (z_offset = 0)
        - Unloading phase (outward motion): lightened contact (z_offset > 0)
        """
        ...
        # Vertical variation for contact modulation (upward only, asymmetric)
        # cos(2π * leg_phase) = 1 at phase 0 (inward, loading) -> firm contact (z_offset = 0)
        # cos(2π * leg_phase) = -1 at phase 0.5 (outward, unloading) -> lightened contact (z_offset = max)
        # Use shifted cosine to create unipolar upward modulation
        vertical_modulation = self.toe_height_variation * 0.5 * (1.0 - np.cos(2 * np.pi * leg_phase))
        foot[2] += vertical_modulation  # Only positive values, lifts slightly during unloading
        
        # Compensate for roll-induced height changes
        current_roll, _, _ = quat_to_euler(self.root_quat)
        
        # Get lateral offset of this foot from body center
        lateral_position = foot[1]
        
        # Roll creates height variation: positive roll tilts right side down, left side up
        # Compensate by adjusting z based on lateral position and roll angle
        roll_compensation = -lateral_position * np.sin(current_roll)
        foot[2] += roll_compensation
