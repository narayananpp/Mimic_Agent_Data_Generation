...
class SKILL_SPLIT_APEX_REENTRY_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Takeoff parameters - enhanced for greater altitude
        self.takeoff_extension = 0.07
        self.takeoff_vx_peak = 2.5
        self.takeoff_vz_peak = 1.8  # Increased from 1.3 for higher apex
        
        # Aerial retraction parameters - reduced to avoid joint limits
        self.retraction_height = 0.09  # Reduced from 0.13
        self.retraction_forward = 0.10
        
        # Distal contact parameters - reduced to avoid joint limits
        self.distal_height_offset = 0.10  # Reduced from 0.22
    ...
    def update_base_motion(self, phase, dt):
        ...
        Enhanced ascent and controlled descent to maintain safe base height.
        """
        
        # Phase 0.0-0.2: Explosive takeoff with enhanced vertical impulse
        if phase < 0.2:
            progress = phase / 0.2
            vx = self.takeoff_vx_peak * np.sin(np.pi * progress / 2.0)
            # Enhanced vertical velocity for greater altitude
            vz = self.takeoff_vz_peak * np.sin(np.pi * progress / 2.0)
            pitch_rate = 0.6 * np.sin(np.pi * progress)  # Reduced from 0.8
            
            self.vel_world = np.array([vx, 0.0, vz])
            self.omega_world = np.array([0.0, pitch_rate, 0.0])
        
        # Phase 0.2-0.5: Extended aerial ascent to apex
        elif phase < 0.5:
            progress = (phase - 0.2) / 0.3
            vx = self.takeoff_vx_peak * (1.0 - 0.15 * progress)
            # Maintain positive vertical velocity longer, gentler decay
            vz = self.takeoff_vz_peak * (1.0 - 1.3 * progress)
            # Reduced pitch rate to decouple from retraction
            pitch_rate = -0.4 * np.sin(np.pi * progress)
            
            self.vel_world = np.array([vx, 0.0, vz])
            self.omega_world = np.array([0.0, pitch_rate, 0.0])
        
        # Phase 0.5-0.6: Distal initial contact with controlled descent
        elif phase < 0.6:
            progress = (phase - 0.5) / 0.1
            vx = self.takeoff_vx_peak * 0.85 * (1.0 - 0.2 * progress)
            # Gradual descent initiation, reduced from -0.6
            vz = -0.3 - 0.2 * progress
            pitch_rate = 0.3 * np.sin(np.pi * progress / 2.0)
            
            self.vel_world = np.array([vx, 0.0, vz])
            self.omega_world = np.array([0.0, pitch_rate, 0.0])
        
        # Phase 0.6-0.8: Momentum dissipation with controlled descent
        elif phase < 0.8:
            progress = (phase - 0.6) / 0.2
            vx = self.takeoff_vx_peak * 0.68 * (1.0 - progress) * np.exp(-2.0 * progress)
            # Reduced descent rate from -0.8 to -0.5
            vz = -0.5 + 0.3 * progress
            pitch_rate = -0.4 * (1.0 - progress)
            
            self.vel_world = np.array([vx, 0.0, vz])
            self.omega_world = np.array([0.0, pitch_rate, 0.0])
        
        # Phase 0.8-1.0: Delayed limb touchdown and stabilization
        else:
            progress = (phase - 0.8) / 0.2
            vx = self.takeoff_vx_peak * 0.08 * (1.0 - progress)**2
            # Smooth gentle descent to zero
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Reduced retraction and distal offsets to maintain joint limits.
        """
        ...
        # Phase 0.2-0.5: Extended aerial ascent with gradual retraction
        elif phase < 0.5:
            progress = (phase - 0.2) / 0.3
            ...
            # Gradual retraction toward body COM with reduced height
            foot[2] += self.retraction_height * blend
            foot[0] += (self.retraction_forward + front_bias) * blend
        
        # Phase 0.5-0.8: Distal contact and momentum dissipation
        elif phase < 0.8:
            progress = (phase - 0.5) / 0.3
            
            # Graduated retraction: begin extending slightly while maintaining distal contact
            retraction_blend = 1.0 - 0.3 * progress
            foot[2] += self.retraction_height * retraction_blend
            
            # Reduced distal elevation with smooth decay
            distal_extra = self.distal_height_offset * (1.0 - 0.6 * progress)
            foot[2] += distal_extra
            ...
            foot[2] -= 0.012 * np.sin(2.0 * np.pi * progress)
        
        # Phase 0.8-1.0: Delayed full limb touchdown and stabilization
        else:
            progress = (phase - 0.8) / 0.2
            
            # Start from retained retraction height plus remaining distal offset
            start_height = self.retraction_height * 0.7 + self.distal_height_offset * 0.4
