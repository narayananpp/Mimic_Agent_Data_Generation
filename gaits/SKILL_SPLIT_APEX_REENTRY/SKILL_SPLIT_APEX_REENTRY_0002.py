...
class SKILL_SPLIT_APEX_REENTRY_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Takeoff parameters (reduced to avoid joint limits)
        self.takeoff_extension = 0.07  # Reduced from 0.15 to prevent joint limit violations
        self.takeoff_vx_peak = 2.5  # Forward velocity at takeoff
        self.takeoff_vz_peak = 1.3  # Reduced from 3.0 to limit peak height
        
        # Aerial retraction parameters (reduced to avoid joint limits)
        self.retraction_height = 0.13  # Reduced from 0.25 to prevent extreme flexion
        self.retraction_forward = 0.10  # Slightly reduced for conservative kinematics
        
        # Distal contact parameters (reduced height offset)
        self.distal_height_offset = 0.06  # Reduced from 0.18 to allow base descent
        self.distal_slide_distance = 0.20  # Rearward drift during momentum dissipation
        
        # Landing parameters
        self.landing_settle_height = 0.04  # Final settling displacement
    ...
    def update_base_motion(self, phase, dt):
        ...
        # Phase 0.2-0.35: Aerial ascent to apex (shortened from 0.45)
        elif phase < 0.35:
            ...
            vz = self.takeoff_vz_peak * (1.0 - progress) - 1.5 * progress
        ...
        # Phase 0.35-0.5: Distal initial contact (descent begins)
        elif phase < 0.5:
            progress = (phase - 0.35) / 0.15
            # Forward velocity begins to decrease
            vx = self.takeoff_vx_peak * 0.8 * (1.0 - 0.4 * progress)
            # Descending with controlled rate
            vz = -1.2 - 0.3 * progress
        ...
        # Phase 0.5-0.75: Momentum dissipation
        elif phase < 0.75:
            progress = (phase - 0.5) / 0.25
            # Rapid forward velocity decay (friction from distal contact)
            vx = self.takeoff_vx_peak * 0.48 * (1.0 - progress) * np.exp(-2.5 * progress)
            # Controlled descent rate, approaching zero
            vz = -1.5 + 1.2 * progress
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        if phase < 0.2:
            progress = phase / 0.2
            # Reduced downward extension to avoid joint limits
            foot[2] -= self.takeoff_extension * np.sin(np.pi * progress / 2.0)
            # Slight rearward motion during push
            foot[0] -= 0.025 * progress
        
        # Phase 0.2-0.35: Aerial ascent with retraction (shortened)
        elif phase < 0.35:
            progress = (phase - 0.2) / 0.15
            # Smooth blending from takeoff extension to retraction
            blend = progress**2 * (3.0 - 2.0 * progress)  # Cubic smoothstep
            
            # Decay takeoff extension smoothly
            extension_decay = self.takeoff_extension * (1.0 - blend)
        ...
        # Phase 0.35-0.75: Distal contact (partial, sliding, extended duration)
        elif phase < 0.75:
            progress = (phase - 0.35) / 0.4
            # Reduced main limb elevation for distal-only contact
            distal_elevation = self.distal_height_offset * (1.0 - progress * 0.7)
            foot[2] += distal_elevation
            ...
            # Subtle vertical oscillation representing tangential sliding dynamics
            foot[2] -= 0.02 * np.sin(2.0 * np.pi * progress)
        
        # Phase 0.75-1.0: Full limb touchdown and stabilization
        else:
            progress = (phase - 0.75) / 0.25
            # Smooth transition from distal position to full stance
            remaining_height = self.distal_height_offset * 0.3 * (1.0 - progress)
            ...
            foot[2] -= self.landing_settle_height * (1.0 - progress) * np.exp(-4.0 * progress)
