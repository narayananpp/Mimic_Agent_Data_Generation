...
class SKILL_AUGER_DRILL_DESCENT_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.radial_extension = 1.2  # Reduced from 1.3 to stay within workspace
        for leg in self.leg_names:
            ...
            # Empirical compensation: raise z by ~15% of original depth to account for
            # the geometric constraint that total leg length is fixed
            z_compensation = 0.15 * abs(orig_z)
        ...
        self.descent_rate = -0.12  # Constant downward velocity (m/s), moderate to avoid workspace limits
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Slight dynamic adjustment compensates for descent to maintain ground contact feasibility.
        """
        # Start with base extended foot position (already compensated for radial extension)
        foot = self.base_feet_pos_body[leg_name].copy()
        
        # Dynamic vertical adjustment: as body descends, gradually raise body-frame foot z
        # to maintain reachability and prevent ground penetration as geometry changes
        # This small adjustment tracks the descent without altering the fundamental stance
        descent_compensation = -self.descent_rate * self.t * 0.1  # Small upward drift in body frame
