...
class SKILL_REVERSE_HELIX_SINK_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Compute nominal height from foot positions to ensure ground contact at initialization
        # Feet in body frame have negative z (below base), so nominal_height = -avg(foot_z_body)
        foot_z_offsets = [foot_pos[2] for foot_pos in initial_foot_positions_body.values()]
        self.nominal_height = -np.mean(foot_z_offsets)
        
        # Sanity check: typical quadruped heights are 0.24-0.32 m
        if self.nominal_height < 0.24 or self.nominal_height > 0.32:
            self.nominal_height = 0.27  # Safe fallback
        ...
        self.min_height = max(self.min_height, 0.20)  # Safety floor to prevent joint limits
