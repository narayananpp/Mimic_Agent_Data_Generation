...
class SKILL_PENDULUM_ROCK_ADVANCE_MotionGenerator(BaseMotionGenerator):
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        # Downward side needs less negative z (lifted in body frame) to prevent penetration
        if is_right_leg:
            # Right legs: compensate for positive (right) roll
            # When roll_angle > 0, right side is down, needs positive compensation
            z_compensation = lateral_distance * max(0, roll_angle)
        else:
            # Left legs: compensate for negative (left) roll
            # When roll_angle < 0, left side is down, needs positive compensation
            z_compensation = lateral_distance * max(0, -roll_angle)
