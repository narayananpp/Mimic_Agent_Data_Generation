...
class SKILL_BUTTERFLY_KICK_GLIDE_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.sweep_height_rear = 0.02  # Vertical lift during recovery (rear legs, minimal)
        self.stance_depth_front = 0.03  # Downward extension during power stroke (front legs only)
        self.stance_depth_rear = 0.0  # No depression for rear legs to avoid ground penetration
        
        # Base foot positions (BODY frame) with vertical offset for rear legs
        self.base_feet_pos_body = {}
        for k, v in initial_foot_positions_body.items():
            pos = v.copy()
            # Add vertical clearance offset to rear legs to prevent penetration
            if k.startswith('R'):
                pos[2] += 0.04  # Lift rear legs baseline position
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Front legs: depression during power stroke, lift during recovery
        Rear legs: minimal depression, reduced lift for ground contact stability
        """
        ...
        if leg_phase < 0.5:
            ...
            # Vertical: press downward during compression (front legs only)
