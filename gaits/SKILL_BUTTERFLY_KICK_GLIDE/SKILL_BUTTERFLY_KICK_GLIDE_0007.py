...
class SKILL_BUTTERFLY_KICK_GLIDE_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Leg motion parameters - tuned to maintain contact while avoiding penetration
        ...
        self.stance_depth_rear = 0.018  # Reduced downward extension for rear legs to avoid penetration
        
        # Base foot positions (BODY frame) with minimal safety offset for rear legs
        self.base_feet_pos_body = {}
        for k, v in initial_foot_positions_body.items():
            pos = v.copy()
            # Add minimal vertical clearance offset to rear legs only
            if k.startswith('R'):
                pos[2] += 0.012  # Small safety margin to prevent penetration
        ...
        # Base motion parameters - reduced vertical amplitude to maintain contact
        self.vx_forward = 0.8  # Forward velocity magnitude
        self.vz_amplitude = 0.08  # Reduced vertical oscillation amplitude
        self.pitch_rate_amplitude = 1.0  # Reduced pitch angular velocity amplitude (rad/s)
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Rear legs: reduced depression to avoid penetration, minimal lift to maintain ground contact
        """
