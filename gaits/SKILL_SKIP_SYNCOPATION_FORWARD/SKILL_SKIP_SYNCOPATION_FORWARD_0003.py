...
class SKILL_SKIP_SYNCOPATION_FORWARD_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.step_length = 0.14  # Reduced from 0.16
        self.rear_bounce_height = 0.055  # Reduced from 0.07
        self.front_bounce_height = 0.05  # Reduced from 0.06
        self.glide_extension_height = 0.065  # Reduced from 0.09
        ...
        self.rear_bounce_pitch_rate = -0.65  # Reduced from -1.0
        self.front_bounce_pitch_rate = 0.85  # Reduced from 1.2
    ...
    def update_base_motion(self, phase, dt):
        ...
        if 0.0 <= phase < 0.15:
            ...
            pitch_rate = -0.35 * glide_progress  # Reduced from -0.5
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        if 0.0 <= phase < 0.15:
            if is_rear:
                ...
                foot[0] -= self.step_length * 0.2 * np.sin(np.pi * progress * 0.5)  # Reduced from 0.25
            else:  # Front legs
                # Swing forward and upward with smooth curve
                progress = phase / 0.15
                swing_curve = np.sin(np.pi * progress)
                foot[0] += self.step_length * 0.45 * swing_curve  # Reduced from 0.6
        ...
        # Phase 3: Front brief contact [0.35, 0.4]
            ...
            if is_front:
                ...
                foot[0] += self.step_length * 0.32  # Reduced from 0.4
                ...
                foot[0] -= self.step_length * 0.22 * (1.0 - smooth_swing * 0.4)
        ...
        # Phase 4: Front double-bounce [0.4, 0.55]
            ...
            if is_front:
                ...
                foot[0] += self.step_length * 0.25  # Reduced from 0.3
            else:  # Rear legs
                # Swing forward through body
                progress = (phase - 0.4) / 0.15
                swing_curve = np.sin(np.pi * progress)
                foot[0] += self.step_length * 0.35 * progress  # Reduced from 0.5
        ...
        # Phase 6: Syncopated landing [0.8, 1.0]
            ...
            if is_fl or is_rr:
                ...
                if phase < 0.88:
                    ...
                    foot[0] += self.step_length * 0.16 * (1.0 - progress * 0.25)
                    foot[2] += self.front_bounce_height * 0.45 * smooth_hold
                else:
                    # Landing phase with smooth descent - cosine smoothing
                    progress = (phase - 0.88) / 0.12
                    descent_curve = 0.5 * (1.0 - np.cos(np.pi * progress))
                    foot[0] += self.step_length * 0.08 * (1.0 - progress)
