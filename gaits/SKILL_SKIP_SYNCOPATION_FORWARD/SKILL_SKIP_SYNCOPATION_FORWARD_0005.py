...
class SKILL_SKIP_SYNCOPATION_FORWARD_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        self.bounce_height = 0.06
        self.glide_extension = 0.08
        self.tuck_height = 0.08
        
        # Velocity parameters (integrated to produce ~0.7 body lengths per cycle)
        self.rear_bounce_vx = 0.8
        self.rear_bounce_vz = 0.6
        ...
        self.front_contact_vz = -0.2
        
        self.front_bounce_vx = 0.6
        self.front_bounce_vz = 0.5
    ...
    def update_base_motion(self, phase, dt):
        ...
        Each phase prescribes velocity and angular rate for kinematic integration.
        """
        ...
        # Phase 0.15-0.35: Long glide forward
            ...
            vx = self.glide_vx
            vy = 0.0
            # Ballistic arc: initially upward, then downward
        ...
        # Phase 0.8-0.9: Left diagonal pair lands
            ...
            vy = -0.05 * np.sin(np.pi * landing_progress)
            vz = -0.3 * (1 - landing_progress)
            roll_rate = -0.4 * np.sin(np.pi * landing_progress)
            pitch_rate = 0.0
            yaw_rate = 0.1 * np.sin(np.pi * landing_progress)
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        if leg_name.startswith('FL'):
            if phase < 0.15:
                ...
                progress = phase / 0.15
                foot[0] += 0.05 * progress
                foot[2] += self.tuck_height * np.sin(np.pi * progress)
            elif phase < 0.35:
                # Swing: extend forward for contact
                progress = (phase - 0.15) / 0.2
                foot[0] += 0.05 + self.glide_extension * progress
                foot[2] += self.tuck_height * (1 - progress * 0.5)
            elif phase < 0.4:
                # Stance: brief contact
                foot[0] += 0.05 + self.glide_extension
                foot[2] += self.tuck_height * 0.5
            elif phase < 0.55:
                # Stance: double-bounce
                bounce_phase = (phase - 0.4) / 0.15
                foot[0] += 0.05 + self.glide_extension - 0.03 * bounce_phase
                foot[2] += self.tuck_height * 0.5 - self.bounce_height * np.abs(np.sin(2 * np.pi * bounce_phase))
            elif phase < 0.8:
                # Swing: retract and prepare for landing
                progress = (phase - 0.55) / 0.25
                foot[0] += 0.02 - self.step_length * progress * 0.5
                foot[2] += self.step_height * np.sin(np.pi * (0.3 + 0.7 * progress))
            else:
                # Stance: landed (left diagonal at 0.8)
                landing_progress = (phase - 0.8) / 0.2
                foot[0] += 0.02 - self.step_length * 0.5 - 0.02 * landing_progress
                foot[2] -= 0.01 * (1 - np.cos(np.pi * min(landing_progress * 5, 1)))
        ...
        # Front Right (FR)
            ...
            if phase < 0.15:
                ...
                # Swing: retract and prepare for delayed landing
                progress = (phase - 0.55) / 0.35
                foot[0] += 0.02 - self.step_length * progress * 0.6
                foot[2] += self.step_height * np.sin(np.pi * (0.2 + 0.8 * progress))
            else:
                # Stance: landed (right diagonal at 0.9)
                landing_progress = (phase - 0.9) / 0.1
                foot[0] += 0.02 - self.step_length * 0.6 - 0.02 * landing_progress
                foot[2] -= 0.01 * (1 - np.cos(np.pi * min(landing_progress * 5, 1)))
        ...
        # Rear Left (RL)
            ...
            if phase < 0.15:
                ...
                foot[2] -= self.bounce_height * np.abs(np.sin(2 * np.pi * bounce_phase))
            elif phase < 0.35:
                ...
                progress = (phase - 0.35) / 0.2
                foot[0] -= 0.02 + self.glide_extension - (self.glide_extension + self.step_length) * progress
                foot[2] += self.step_height * 0.8 * (1 - progress)
            elif phase < 0.8:
                # Swing: continue forward, prepare for landing
                progress = (phase - 0.55) / 0.25
                foot[0] -= self.step_length + 0.03 * progress
                foot[2] += self.step_height * 0.5 * (1 - progress)
            else:
                # Stance: landed (left diagonal at 0.8)
                landing_progress = (phase - 0.8) / 0.2
                foot[0] -= self.step_length + 0.03 + 0.02 * landing_progress
                foot[2] -= 0.01 * (1 - np.cos(np.pi * min(landing_progress * 5, 1)))
        ...
        # Rear Right (RR)
            ...
            if phase < 0.15:
                ...
                # Swing: continue forward, delayed landing prep
                progress = (phase - 0.55) / 0.35
                foot[0] -= self.step_length + 0.03 * progress
                foot[2] += self.step_height * 0.5 * (1 - progress * 0.9)
            else:
                # Stance: landed (right diagonal at 0.9)
                landing_progress = (phase - 0.9) / 0.1
                foot[0] -= self.step_length + 0.03 + 0.02 * landing_progress
