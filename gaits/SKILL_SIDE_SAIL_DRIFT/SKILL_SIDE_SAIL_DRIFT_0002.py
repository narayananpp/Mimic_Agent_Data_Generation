...
class SKILL_SIDE_SAIL_DRIFT_MotionGenerator(BaseMotionGenerator):
    ...
    def __init__(self, initial_foot_positions_body, leg_names):
        ...
        # Base motion parameters - reduced roll rate to prevent ground penetration
        self.lateral_vel_amplitude = 0.3  # m/s lateral velocity magnitude
        self.roll_rate_amplitude = 0.22  # rad/s roll rate magnitude (reduced from 0.4)
        
        # Roll compensation tracking
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Includes roll compensation to prevent ground penetration.
        """
        ...
        # Compute extension/compaction factor based on phase with unified smooth blending
        if 0.0 <= phase < 0.3:
            ...
            # Transition to left - unified smooth interpolation
            progress = (phase - 0.3) / 0.2
            # Smooth blend from 1 to -1 through 0 using sine wave
            blend_factor = np.cos(np.pi * progress)  # +1 to -1
            if is_right_leg:
                # Interpolate from extended to compact
                lateral_offset = self.lateral_extension * 0.5 * (blend_factor + 1.0) - self.lateral_extension * self.compact_factor * 0.5 * (1.0 - blend_factor)
            else:
                # Interpolate from compact to extended
                lateral_offset = -self.lateral_extension * self.compact_factor * 0.5 * (blend_factor + 1.0) + self.lateral_extension * 0.5 * (1.0 - blend_factor)
            ...
            # Transition to right - unified smooth interpolation
            progress = (phase - 0.8) / 0.2
            # Smooth blend from -1 to +1 through 0
            blend_factor = -np.cos(np.pi * progress)  # -1 to +1
            if is_right_leg:
                # Interpolate from compact to extended
                lateral_offset = -self.lateral_extension * self.compact_factor * 0.5 * (1.0 - blend_factor) + self.lateral_extension * 0.5 * (blend_factor + 1.0)
            else:
                # Interpolate from extended to compact
                lateral_offset = self.lateral_extension * 0.5 * (1.0 - blend_factor) - self.lateral_extension * self.compact_factor * 0.5 * (blend_factor + 1.0)
        
        # Apply lateral offset to y-coordinate (lateral in body frame)
        foot_pos = base_pos.copy()
        foot_pos[1] += lateral_offset
        
        # Roll compensation: when body rolls and legs extend laterally, compensate height
        # to maintain ground contact in world frame
        # Use current tracked roll angle to compute geometric compensation
        if abs(self.current_roll_angle) > 1e-6 and abs(lateral_offset) > 1e-6:
            # When rolling right (positive roll), right legs (positive y offset) drop
            # When rolling left (negative roll), left legs (negative y offset) drop
            # Compensation lifts the extended side in body frame
            z_compensation = abs(lateral_offset * np.sin(self.current_roll_angle))
