...
class SKILL_UNDULATING_CRAWL_TURN_MotionGenerator(BaseMotionGenerator):
    ...
    def compute_body_vertical_displacement(self, phase):
        """
        Compute the body's vertical displacement from neutral due to undulation.
        This represents how much the body frame has moved vertically at a given phase.
        """
        # Body undulation integrates vz over time
        # vz = self.undulation_amplitude_z * np.cos(2 * np.pi * phase) * 2.0
        # Integrate: z = (amplitude * 2.0 / (2 * pi * freq)) * sin(2 * pi * phase)
        # Since we're working in body frame and need relative displacement:
        displacement = self.undulation_amplitude_z * np.sin(2 * np.pi * phase)
        return displacement
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        Stance trajectory: linear rearward sweep with vertical compensation for body undulation
        """
        ...
        if abs(phase_rel) < swing_half:
            ...
            # CRITICAL FIX: Vertical compensation for body undulation during stance
            # When body undulates down, foot must move up in body frame to maintain ground contact
            # Compute body vertical displacement at current phase
            body_z_displacement = self.compute_body_vertical_displacement(phase)
            
            # Compensate: if body moves down (negative displacement), foot moves up in body frame
            # This keeps the foot at constant world height (on ground) during stance
            foot[2] -= body_z_displacement
