...
class SKILL_ELLIPTICAL_LEAN_LOOP_MotionGenerator(BaseMotionGenerator):
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        # Left legs (FL, RL): shift left when rolling right (positive roll)
        # Right legs (FR, RR): shift right when rolling right
        if leg_name.startswith('FL') or leg_name.startswith('RL'):
            foot[1] -= self.lean_compensation * roll  # Negative y is left
        elif leg_name.startswith('FR') or leg_name.startswith('RR'):
            foot[1] += self.lean_compensation * roll  # Positive y is right
