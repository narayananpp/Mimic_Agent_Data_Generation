...
class SKILL_METRONOME_SWAY_ADVANCE_MotionGenerator(BaseMotionGenerator):
    ...
    - Conservative parameters to respect joint limits while maintaining contact
    """

    def __init__(self, initial_foot_positions_body, leg_names):
        self.leg_names = leg_names
        self.freq = 0.6  # Moderate frequency for stable sway motion
        
        # Base foot positions (BODY frame) - use original positions without modification
        self.base_feet_pos_body = {k: v.copy() for k, v in initial_foot_positions_body.items()}
        
        # Motion parameters - balanced to avoid both joint violations and airborne states
        self.max_roll_angle = np.deg2rad(15.0)  # Conservative 15° roll angle
        self.leg_compression_range = 0.15  # Moderate compression range (±0.075m per leg)
        ...
        # Vertical motion parameters - very small with downward bias
        self.vertical_velocity_amp = 0.02  # Minimal amplitude
        self.vertical_bias = -0.015  # Small downward bias to maintain ground contact
    ...
    def update_base_motion(self, phase, dt):
        ...
        # Vertical velocity: gentle oscillation with downward bias to maintain contact
        # Two cycles per gait cycle (sin(4πφ)) for smooth up-down motion
        vz = self.vertical_bias + self.vertical_velocity_amp * np.sin(4 * np.pi * phase)
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        # Use smooth sinusoidal profile synchronized with roll angle
        if is_right_leg:
        ...
        # Small lateral adjustment to accommodate roll geometry
        # Compressed side moves slightly inward, extended side slightly outward
        lateral_adjustment = 0.035 * np.sin(2 * np.pi * phase)
        if is_right_leg:
