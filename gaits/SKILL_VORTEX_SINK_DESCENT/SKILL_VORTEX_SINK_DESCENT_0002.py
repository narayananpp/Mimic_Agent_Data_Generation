...
class SKILL_VORTEX_SINK_DESCENT_MotionGenerator(BaseMotionGenerator):
    ...
    def compute_foot_position_body_frame(self, leg_name, phase):
        ...
        # Z-position adjustment: legs extend downward relative to body to maintain ground contact as base descends
        # As base descends in world frame, feet must move UP in body frame to stay at ground level
        # Compute accumulated descent based on velocity profile
        z_compensation = self._compute_accumulated_descent(phase)
        
        # CRITICAL: ADD compensation to move feet upward in body frame
        foot_pos[2] += z_compensation
        
        return foot_pos

    def _compute_accumulated_descent(self, phase):
        """
        Compute total accumulated descent at given phase based on velocity profile.
        This matches the integrated downward displacement of the base.
        """
        # Period duration for one full cycle
        period = 1.0 / self.freq
        
        # Phase 0.0-0.25: minimal descent at vz=-0.05
        if phase <= 0.25:
            descent = abs(-0.05) * phase * period
        elif phase <= 0.75:
            # Descent from phase 0.0-0.25
            descent_phase1 = abs(-0.05) * 0.25 * period
            # Descent from phase 0.25 to current phase at vz=-0.25
            descent_phase2 = abs(self.vz_descent) * (phase - 0.25) * period
            descent = descent_phase1 + descent_phase2
        else:
            # Descent from phase 0.0-0.25
            descent_phase1 = abs(-0.05) * 0.25 * period
            # Descent from phase 0.25-0.75
            descent_phase2 = abs(self.vz_descent) * 0.5 * period
            # Tapered descent from phase 0.75 to current phase
            # Using average velocity with linear taper
            phase_in_taper = phase - 0.75
            duration_in_taper = phase_in_taper * period
            # Average taper factor over the interval
            avg_taper = (1.0 + (1.0 - phase) / 0.25) / 2.0
            descent_phase3 = abs(self.vz_descent) * avg_taper * phase_in_taper * period
            descent = descent_phase1 + descent_phase2 + descent_phase3
